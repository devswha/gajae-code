...
export const GJC_TMUX_HUD_LEADER_PANE_ENV = "GJC_TMUX_HUD_LEADER_PANE";
const GJC_TMUX_HUD_OWNER_ENV = "GJC_TMUX_HUD_OWNER";
...
export function isHudWatchPane(pane: TmuxPaneSnapshot): boolean {
	...
	return (
		...
		(/\bgjc(?:\.js)?\b/.test(command) || /\bnode\b/.test(command))
	);
}
...
function parseShellEnvAssignment(command: string, key: string): string | undefined {
	...
	const match = command.match(
		new RegExp(`(?:^|\\s)(?:'${escapedKey}=([^']*)'|${escapedKey}=(?:'((?:'\\\\''|[^'])*)'|([^\\s]+)))`),
	);
	const fallbackMatch = match ? null : command.match(new RegExp(`(?:^|[\\s'])${escapedKey}=([^'\\s]+)`));
...
export function readHudPaneOwner(pane: TmuxPaneSnapshot): HudPaneOwner {
	...
	return {
		sessionId: parseShellEnvAssignment(command, "GJC_SESSION_ID"),
		leaderPaneId: parseShellEnvAssignment(command, GJC_TMUX_HUD_LEADER_PANE_ENV),
	};
}
...
export function buildHudResizeHookName(sessionId: string, windowId: string): string {
	return ["gjc_hud_resize", normalizeTmuxHookToken(sessionId), normalizeTmuxHookToken(windowId)].join("_");
}
...
export function readHudResizeHookContext(
	...
	try {
		return parseHudResizeHookContext(
			execTmuxSync(["display-message", "-p", "-t", currentPaneId, "#{session_id}\t#{window_id}"]),
		);
	} catch {
		return null;
	}
...
export function buildHudWatchCommand(
	...
	const safePreset = preset === "minimal" || preset === "focused" || preset === "full" ? ` --preset=${preset}` : "";
	const safeSessionId = typeof sessionId === "string" ? sessionId.trim() : "";
	const safeGjcRoot = typeof gjcRoot === "string" ? gjcRoot : "";
	const safeLeaderPaneId = typeof leaderPaneId === "string" ? leaderPaneId.trim() : "";
	const envPrefix = buildEnvPrefix({
		GJC_SESSION_ID: safeSessionId,
		[GJC_TMUX_HUD_OWNER_ENV]: "1",
		[GJC_TMUX_HUD_LEADER_PANE_ENV]: safeLeaderPaneId,
		GJC_ROOT: safeGjcRoot,
	});
	return `exec ${envPrefix}${shellEscapeSingle(process.execPath)} ${shellEscapeSingle(gjcBin)} hud --watch${safePreset}`;
}
...
export function readActiveTmuxPaneId(execTmuxSync: TmuxExecSync = defaultExecTmuxSync): string | null {
	try {
...
export function createHudWatchPane(
	...
	const heightLines =
		Number.isFinite(options.heightLines) && (options.heightLines ?? 0) > 0
...
export function killTmuxPane(paneId: string, execTmuxSync: TmuxExecSync = defaultExecTmuxSync): boolean {
...
export function resizeTmuxPane(
	...
	if (!paneId.startsWith("%")) return false;
	const height = Number.isFinite(heightLines) && heightLines > 0 ? Math.floor(heightLines) : HUD_TMUX_HEIGHT_LINES;
	try {
