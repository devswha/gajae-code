/**
 * Session Lifecycle Manager for gajae-code
 *
...
import { appendFile, mkdir, readFile, rm, unlink, writeFile } from "fs/promises";
import { dirname, join } from "path";
import { getStateFilePath } from "../mcp/state-paths.js";
import { gjcLogsDir, gjcRoot, gjcStateDir, sameFilePath } from "../utils/paths.js";

export interface SessionState {
	session_id: string;
	native_session_id?: string;
	// Native session replacement metadata used when Codex /new swaps the native
	// session while the original GJC launch wrapper is still responsible for
	// archiving/cleanup.
	previous_native_session_id?: string;
	native_session_switched_at?: string;
	owner_gjc_session_id?: string;
	started_at: string;
	cwd: string;
	pid: number;
...
function sessionPath(cwd: string): string {
	return join(gjcStateDir(cwd), SESSION_FILE);
}

function historyPath(cwd: string): string {
	return join(gjcLogsDir(cwd), HISTORY_FILE);
}

async function removeDeadSessionHudState(cwd: string, sessionIds: Array<string | undefined>): Promise<void> {
...
export async function resetSessionMetrics(cwd: string, sessionId?: string): Promise<void> {
	const gjcDir = gjcRoot(cwd);
	const stateDir = gjcStateDir(cwd);
	await mkdir(gjcDir, { recursive: true });
	await mkdir(stateDir, { recursive: true });

	const now = new Date().toISOString();
	await writeFile(
		join(gjcDir, "metrics.json"),
		JSON.stringify(
			{
				total_turns: 0,
				session_turns: 0,
...
export function isSessionStateUsable(
	...
	const hasLinuxIdentityMetadata = typeof state.pid_start_ticks === "number" || typeof state.pid_cmdline === "string";
...
function createSessionState(
	...
	const tmuxSessionName =
		typeof options.tmuxSessionName === "string" && options.tmuxSessionName.trim()
	...
	const nativeSessionSwitchedAt =
		typeof options.nativeSessionSwitchedAt === "string" && options.nativeSessionSwitchedAt.trim()
			...
			: undefined;
	const ownerGjcSessionId =
		typeof options.ownerGjcSessionId === "string" && options.ownerGjcSessionId.trim()
			? options.ownerGjcSessionId.trim()
			: undefined;
	...
	return {
		...
		...(ownerGjcSessionId ? { owner_gjc_session_id: ownerGjcSessionId } : {}),
 ...
 */
export function isSessionStale(state: SessionState, options: SessionStaleCheckOptions = {}): boolean {
...
export async function writeSessionStart(
	...
	const stateDir = gjcStateDir(cwd);
	await mkdir(stateDir, { recursive: true });
	const pid = Number.isInteger(options.pid) && options.pid && options.pid > 0 ? options.pid : process.pid;
	const platform = options.platform ?? process.platform;
	const linuxIdentity = platform === "linux" ? readLinuxProcessIdentity(pid) : null;
	const state = createSessionState(cwd, sessionId, pid, platform, linuxIdentity, {
		...
		ownerGjcSessionId: options.ownerGjcSessionId,
...
function getGjcLaunchSessionId(state: SessionState): string | undefined {
	if (state.session_id.startsWith("gjc-")) return state.session_id;
	if (typeof state.owner_gjc_session_id === "string" && state.owner_gjc_session_id.startsWith("gjc-")) {
		return state.owner_gjc_session_id;
	}
	return undefined;
}

/**
 * Reconcile a native/Codex SessionStart with the canonical GJC launch session.
 ...
 * belongs to an GJC launch wrapper, retain that wrapper as owner for later
...
export async function reconcileNativeSessionStart(
	...
	const existingNativeSessionId =
		typeof existing.native_session_id === "string" ? existing.native_session_id.trim() : "";
	if (existingNativeSessionId && existingNativeSessionId !== nativeSessionId) {
		const ownerGjcSessionId = getGjcLaunchSessionId(existing);
		if (ownerGjcSessionId) {
			const pid = Number.isInteger(options.pid) && options.pid && options.pid > 0 ? options.pid : process.pid;
			const nowIso = new Date().toISOString();
			await appendToLog(cwd, {
				event: "native_session_replaced",
				session_id: ownerGjcSessionId,
				...(existing.session_id !== ownerGjcSessionId ? { active_session_id: existing.session_id } : {}),
	...
	const state = createSessionState(cwd, existing.session_id, pid, platform, linuxIdentity, {
		...
		ownerGjcSessionId: existing.owner_gjc_session_id,
...
export async function writeSessionEnd(cwd: string, sessionId: string): Promise<void> {
	...
	const ownsCurrentSessionFile =
		state == null ||
		state.session_id === sessionId ||
		state.native_session_id === sessionId ||
		state.owner_gjc_session_id === sessionId;

	// Archive to session history
	const logsDir = gjcLogsDir(cwd);
	...
	const historyEntry = {
		...
		...(ownsCurrentSessionFile && state?.owner_gjc_session_id === sessionId && state?.session_id
...
export async function appendToLog(cwd: string, entry: Record<string, unknown>): Promise<void> {
	...
	const logFile = join(logsDir, `gjc-${date}.jsonl`);
