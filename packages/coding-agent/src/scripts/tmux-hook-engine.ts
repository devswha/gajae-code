...
export const DEFAULT_MARKER = "[GJC_TMUX_INJECT]";
const PLACEHOLDER_TARGET_VALUES = new Set(["replace-with-tmux-pane-id", "replace-with-tmux-session-name", "unset"]);
...
export function normalizeTmuxHookConfig(raw: any): any {
	...
	const targetValue =
		raw.target && typeof raw.target === "object" && typeof raw.target.value === "string"
	...
	const marker = typeof raw.marker === "string" && raw.marker.trim() !== "" ? raw.marker : DEFAULT_MARKER;
	...
	return {
		...
		skip_if_scrolling: raw.skip_if_scrolling !== false,
	};
}
...
export function evaluateInjectionGuards({
	...
	if (!skipQuotaChecks) {
		...
		const legacySessionCounts =
			state.session_counts && typeof state.session_counts === "object" ? state.session_counts : {};
		...
		const count =
			paneKeyNorm && typeof paneCounts[paneKeyNorm] === "number"
				? paneCounts[paneKeyNorm]
				: typeof legacySessionCounts[sessionKeyNorm] === "number"
					? legacySessionCounts[sessionKeyNorm]
					: 0;
...
function isHudStartCommand(startCommand: string): boolean {
	return /\bgjc\b.*\bhud\b.*--watch/i.test(startCommand);
}
...
export function paneHasActiveTask(captured: any): boolean {
	const tail = normalizePaneLines(captured)
		.map(line => line.trim())
		.slice(-40);
...
export function buildSendKeysArgv({ paneTarget, prompt, dryRun, submitKeyPresses = 2 }: any): any {
	...
	const submitArgv = Array.from({ length: pressCount }, () => ["send-keys", "-t", paneTarget, "C-m"]);
