...
const MARKDOWN_FENCE_PATTERN = /^(?: {0,3})(?<marker>`{3,}|~{3,})(?<suffix>[^\r\n]*)$/;
...
function advanceMarkdownCommentDepth(activeDepth: number, line: string): number {
...
function advanceMarkdownFenceState(activeFence: MarkdownFenceState | null, line: string): MarkdownFenceState | null {
...
function isMarkdownCommentLine(state: MarkdownVisibilityState, line: string): boolean {
	return state.commentDepth > 0 || MARKDOWN_COMMENT_LINE_PATTERN.test(line);
}

export function inspectMarkdownLine(state: MarkdownVisibilityState, line: string): MarkdownLineInspection {
	if (state.fence) {
		return {
			scanState: "fenced",
			visibleText: "",
