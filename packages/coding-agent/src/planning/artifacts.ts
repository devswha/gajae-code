...
export function readPlanningArtifacts(cwd: string): PlanningArtifacts {
	const plansDir = join(cwd, ".gjc", "plans");
	const specsDir = join(cwd, ".gjc", "specs");
	...
	return {
		...
		deepInterviewSpecPaths: readMatchingPaths(specsDir, DEEP_INTERVIEW_SPEC_PATTERN).filter(
			path => parsePlanningArtifactFileName(path)?.kind === "deep-interview",
		),
	};
}
...
function selectPlanningArtifactsBase(artifacts: PlanningArtifacts, prdPath?: string): PlanningArtifactSelectionBase {
	const requestedPrdPath = prdPath == null ? null : resolveRequestedPrdPath(artifacts, prdPath);
	const selectedPrdPath = prdPath == null ? selectLatestPlanningArtifactPath(artifacts.prdPaths) : requestedPrdPath;
	const slug = selectedPrdPath ? planningArtifactSlug(selectedPrdPath, "prd") : null;
...
function resolveRequestedPrdPath(artifacts: PlanningArtifacts, rawPrdPath: string): string | null {
...
function resolveRelativePathWithinRoot(baseDir: string, rawPath: string, rootDir: string): string | null {
...
function selectPlanningArtifacts(artifacts: PlanningArtifacts, prdPath?: string): LatestPlanningArtifactSelection {
...
function extractApprovedRepositoryContextSection(
	sourcePath: string,
	content: string,
...
export function selectLatestPlanningArtifacts(artifacts: PlanningArtifacts): LatestPlanningArtifactSelection {
...
const TEAM_LAUNCH_HINT_PATTERN_SOURCE = String.raw`(?<command>(?:gjc\s+team|\$team)\s+(?<ralph>ralph\s+)?(?<count>\d+)(?::(?<role>[a-z][a-z0-9-]*))?\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))`;
const RALPH_LAUNCH_HINT_PATTERN_SOURCE = String.raw`(?<command>(?:gjc\s+ralph|\$ralph)\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))`;
...
function normalizeLaunchHintCommandFromMatch(
	...
	if (mode === "team") {
		...
		const prefix = /^\$team\b/i.test(rawCommand) ? "$team" : "gjc team";
	...
	const prefix = /^\$ralph\b/i.test(rawCommand) ? "$ralph" : "gjc ralph";
	return `${prefix} ${taskToken}`;
}

function normalizeLaunchHintCommand(mode: "team" | "ralph", command: string | undefined): string | undefined {
...
function collectLaunchHintMatches(content: string, mode: "team" | "ralph"): RegExpMatchArray[] {
...
function hasRequestedTeamLaunchSignature(options: ApprovedExecutionLaunchHintReadOptions): boolean {
	return options.workerCount != null || options.agentType != null || options.linkedRalph != null;
}
...
function sameTeamLaunchSignatureMatch(anchorHint: ApprovedExecutionLaunchHint, match: RegExpMatchArray): boolean {
	const groups = match.groups;
	if (!groups) {
		return false;
	}
...
export function readApprovedExecutionLaunchHintOutcome(
	...
	if (!normalizedTask && !normalizedCommand) {
		...
		const fallback = resolveOlderReusableSameLineageHint(cwd, mode, artifacts, latestPrdPath, latestOutcome.hint);
		if (fallback.status === "ambiguous") {
			return { status: "ambiguous" };
		}
		return fallback.status === "resolved" ? { status: "resolved", hint: fallback.hint } : latestOutcome;
	}
	...
	for (const prdPath of orderedPrdPathsNewestFirst(artifacts.prdPaths)) {
		...
		const requestedTeamMatchFilter =
			mode === "team" ? buildRequestedTeamLaunchSignatureMatchFilter(options) : undefined;
		const teamLineageMatchFilter =
			requestedTeamMatchFilter ??
			(mode === "team" && normalizedTask && !normalizedCommand && teamLineageAnchor
