 ...
 */
export async function loadRolePrompt(role: string, promptsDir: string): Promise<string | null> {
...
const ROLE_KEYWORDS: ReadonlyArray<{ role: string; keywords: readonly string[] }> = [
	...
	},
	{
		...
		keywords: ["review", "audit", "quality", "lint", "anti-pattern", "code review", "검토", "리뷰"],
	},
	{
		role: "code-reviewer",
		keywords: ["security", "owasp", "xss", "injection", "cve", "vulnerability", "보안", "취약점"],
	},
	{
		role: "code-simplifier",
		keywords: ["refactor", "simplify", "clean up", "reduce complexity", "consolidate", "리팩터", "단순화"],
	},
];

const IMPLEMENTATION_INTENT =
	/\b(?:add|build|create|fix|implement|make|migrate|repair|ship|support|update|wire)\b|(?:구현|추가|수정|업데이트|지원)/i;
const REVIEW_INTENT = /\b(?:audit|check|inspect|review|validate|verify)\b|(?:검토|리뷰|감사|확인|검증)/i;
const PRIMARY_TEST_INTENT =
	/^(?:add|create|expand|improve|increase|write)\b.*\b(?:tests?|specs?|coverage)\b|^(?:테스트\s*(?:추가|작성)|커버리지\s*추가)/i;
const DOCS_INTENT = /\b(?:docs?|documentation|readme|guide|changelog)\b|(?:문서|가이드|README|변경로그)/i;
const PRIMARY_DOCS_INTENT =
	/^(?:document|draft|write|update)\b.*\b(?:docs?|documentation|readme|guide|changelog)\b|^(?:문서\s*(?:업데이트|작성)|README\s*업데이트|가이드\s*작성)/i;
const DEBUG_INTENT = /\b(?:debug|diagnose|investigate|root cause|trace|bisect)\b|(?:디버그|조사|원인)/i;
const DESIGN_INTENT =
	/\b(?:design|layout|style)\b|\b(?:build|create)\b.*\b(?:ui|component|frontend)\b|(?:디자인|레이아웃|스타일|컴포넌트)/i;
...
const SECURITY_DOMAIN =
	/\b(?:auth|authentication|authorization|cve|injection|owasp|security|vulnerability|xss)\b|(?:보안|인증|인가|취약점)/i;
const LOCAL_EXPLORATION_VERB =
	/\b(?:check|find|inspect|locate|look up|lookup|map|review|search|trace|understand|where(?:\s+is|\s+are)?|which files?|what files?)\b/i;
const LOCAL_EXPLORATION_SUBJECT =
	/\b(?:file|files|symbol|symbols|repo|repository|codebase|path|paths|usage|usages|reference|references|relationship|relationships|wiring|flow|implementation|local)\b/i;
const LOCAL_USAGE_DISCOVERY =
	/\b(?:call sites?|current(?:ly)? use|how\s+we\s+use|integration points?|our usage|where\s+we\s+use)\b/i;
const DEPENDENCY_EVALUATION_SIGNAL =
	/\b(?:dependency|dependencies|package|packages|sdk|sdks|library|libraries|framework|frameworks|crate|crates|npm|pypi|crates\.io|license|licenses|maintenance|download stats?|migration path|vendor)\b/i;
const DEPENDENCY_EVALUATION_VERB =
	/\b(?:adopt|assess|choose|compare|evaluate|recommend|replace|select|swap|upgrade)\b/i;
const DEPENDENCY_EVALUATION_CONTEXT =
	/\b(?:candidate|candidates|comparison|download stats?|license|licenses|maintenance|migration path|options?|replacement|risk|trade-?offs?|upgrade|vendor)\b/i;
const DEPENDENCY_IMPLEMENTATION_SIGNAL =
	/\b(?:adapter|api|call sites?|client|clients|code(?:path|paths)?|endpoint|endpoints|flow|flows|handler|handlers|implementation|imports?|integrat(?:e|ion)|module|modules|refactor|wire)\b/i;
const RESEARCH_SIGNAL =
	/\b(?:official docs?|upstream docs?|vendor docs?|reference|references|api docs?|release notes?|changelog|version(?:ing)?|compatib(?:ility|le)|research)\b/i;
const RESEARCH_VERB = /\b(?:check|consult|investigate|look up|lookup|read|research|review|study|verify)\b/i;
const CHOSEN_TECH_RESEARCH_SIGNAL =
	/\b(?:api|apis|framework|frameworks|library|libraries|sdk|sdks|service|services|tool|tools|vendor)\b/i;
const CHOSEN_TECH_RESEARCH_NEED =
	/\b(?:behavior|best way|configuration|configure|example|examples|feature|features?|how(?:\s+do|\s+to)?|in the wild|lifecycle|option|options|parameter|parameters|usage|what(?:\s+does|\s+is)|when(?:\s+does|\s+should)|why(?:\s+does)?)\b/i;
const DOCS_DELIVERABLE_VERB = /\b(?:add|document|draft|edit|prepare|publish|refresh|revise|update|write)\b/i;
const DOCS_DELIVERABLE_NOUN =
	/\b(?:api docs?|changelog|comments?|documentation|docs?|guide|guides|readme|release notes?)\b/i;

function isLocalExplorationTask(text: string): boolean {
	return (
		(LOCAL_EXPLORATION_VERB.test(text) && LOCAL_EXPLORATION_SUBJECT.test(text)) ||
		(LOCAL_USAGE_DISCOVERY.test(text) &&
			/\b(?:current|currently|dependency|existing|local|our|package|packages|repo|repository|sdk|sdks|library|libraries)\b/i.test(
				text,
			))
	);
}
...
function isImplementationHeavyDependencyTask(text: string): boolean {
	return (
		DEPENDENCY_EVALUATION_SIGNAL.test(text) &&
...
function isDependencyEvaluationTask(text: string): boolean {
	...
	return (
		DEPENDENCY_EVALUATION_SIGNAL.test(text) &&
		(DEPENDENCY_EVALUATION_VERB.test(text) || DEPENDENCY_EVALUATION_CONTEXT.test(text))
	);
}

function isResearchTask(text: string): boolean {
	const docsDrivenResearch =
		RESEARCH_SIGNAL.test(text) &&
		(RESEARCH_VERB.test(text) ||
			/\b(?:compatib(?:ility|le)|official docs?|release notes?|upstream docs?|vendor docs?|version(?:ing)?)\b/i.test(
				text,
			));
	const chosenTechnologyGuidance = CHOSEN_TECH_RESEARCH_SIGNAL.test(text) && CHOSEN_TECH_RESEARCH_NEED.test(text);

	return (
		(docsDrivenResearch || chosenTechnologyGuidance) &&
...
export function routeTaskToRole(
	...
	if (isImplementationHeavyDependencyTask(text)) {
		return {
			...
			confidence: "medium",
			reason:
				"dependency/sdk terms appear inside implementation-heavy replacement work, so using fallback implementation lane",
		};
	}
	...
	if (intent === "implementation" && SECURITY_DOMAIN.test(text)) {
		return {
			...
			confidence: "medium",
			reason:
				"security/auth domain detected but task intent is implementation, so using fallback implementation lane",
		};
	}
