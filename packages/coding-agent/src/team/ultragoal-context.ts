...
export interface UltragoalTeamContext {
	...
	goalsPath: ".gjc/ultragoal/goals.json";
	ledgerPath: ".gjc/ultragoal/ledger.jsonl";
	activeGoalId: string;
...
export interface UltragoalCheckpointGuidance {
	...
	goals_path: ".gjc/ultragoal/goals.json";
	ledger_path: ".gjc/ultragoal/ledger.jsonl";
...
function contextStatePath(teamName: string, cwd: string, teamStateRoot?: string | null): string {
	...
	return join(teamStateRoot ?? resolveCanonicalTeamStateRoot(cwd), "team", teamName, "ultragoal-context.json");
}
...
export function normalizeUltragoalTeamContext(value: unknown): UltragoalTeamContext | null {
	...
	if (raw.goalsPath !== ".gjc/ultragoal/goals.json") return null;
	if (raw.ledgerPath !== ".gjc/ultragoal/ledger.jsonl") return null;
	...
	const activeGoalTitle =
		typeof raw.activeGoalTitle === "string" && raw.activeGoalTitle.trim() !== ""
...
export async function resolveLeaderOwnedUltragoalContext(cwd: string): Promise<UltragoalTeamContext | null> {
	const goalsJsonPath = join(cwd, ".gjc", "ultragoal", "goals.json");
	...
	try {
		...
		const activeGoal = goals.find(
			goal => goal && typeof goal === "object" && (goal as Record<string, unknown>).id === activeGoalId,
		...
		const activeGoalTitle =
			typeof activeGoal?.title === "string" && activeGoal.title.trim() !== "" ? activeGoal.title.trim() : undefined;
		return {
			kind: "leader_owned_ultragoal_context",
			goalsPath: ".gjc/ultragoal/goals.json",
			ledgerPath: ".gjc/ultragoal/ledger.jsonl",
...
export function buildUltragoalCheckpointGuidance(context: UltragoalTeamContext): UltragoalCheckpointGuidance {
	const goalId = context.activeGoalId;
	const intermediateStoryCommand = `gjc ultragoal checkpoint --goal-id ${goalId} --status complete --evidence "<team evidence mentioning .gjc/ultragoal and ${goalId}>" --codex-goal-json <fresh-active-get_goal-json-or-path>`;
	const finalStoryCommand = `gjc ultragoal checkpoint --goal-id ${goalId} --status complete --evidence "<team evidence mentioning .gjc/ultragoal and ${goalId}>" --codex-goal-json <fresh-complete-get_goal-json-or-path> --quality-gate-json <quality-gate-json-or-path>`;
	return {
		goal_id: goalId,
		...
		evidence_requirements: [
			...
			"evidence mentions .gjc/ultragoal artifacts",
		...
		command_templates: {
			...
			per_story: `gjc ultragoal checkpoint --goal-id ${goalId} --status complete --evidence "<team evidence mentioning .gjc/ultragoal and ${goalId}>" --codex-goal-json <fresh-matching-get_goal-json-or-path>`,
			completed_wrong_legacy_goal_blocker: `gjc ultragoal checkpoint --goal-id ${goalId} --status blocked --evidence "<completed legacy Codex goal blocks this ultragoal story>" --codex-goal-json <fresh-completed-wrong-get_goal-json-or-path>`,
		},
	};
}
...
export function renderLeaderOwnedUltragoalContextSection(
	...
	return [
		...
		"- Workers must not create worker Ultragoal ledgers, mutate `.gjc/ultragoal`, auto-launch Team from Ultragoal, or claim shell commands changed Codex goal state.",
...
export function renderUltragoalCheckpointGuidanceText(context: UltragoalTeamContext | null | undefined): string[] {
	if (!context) return [];
	const guidance = buildUltragoalCheckpointGuidance(context);
	return [
		...
		"  worker_boundary: workers do not own Ultragoal goal state or mutate .gjc/ultragoal artifacts",
