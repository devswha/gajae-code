...
import { CODEX_BYPASS_FLAG, CONFIG_FLAG, LONG_CONFIG_FLAG, MADMAX_FLAG } from "../cli/constants.js";
import {
...
import { resolveGjcCliEntryPath } from "../utils/paths.js";
import {
...
import { GJC_TMUX_HUD_OWNER_ENV } from "../hud/reconcile.js";
import { GJC_TMUX_HUD_LEADER_PANE_ENV } from "../hud/tmux.js";

const GJC_INSTANCE_OPTION = "@gjc_instance_id";
const GJC_PANE_INSTANCE_OPTION = "@gjc_pane_instance_id";
...
const INJECTION_MARKER = "[GJC_TMUX_INJECT]";
const MODEL_INSTRUCTIONS_FILE_KEY = "model_instructions_file";
const GJC_BYPASS_DEFAULT_SYSTEM_PROMPT_ENV = "GJC_BYPASS_DEFAULT_SYSTEM_PROMPT";
const GJC_MODEL_INSTRUCTIONS_FILE_ENV = "GJC_MODEL_INSTRUCTIONS_FILE";
const GJC_TEAM_WORKER_CLI_ENV = "GJC_TEAM_WORKER_CLI";
const GJC_TEAM_WORKER_CLI_MAP_ENV = "GJC_TEAM_WORKER_CLI_MAP";
const GJC_TEAM_WORKER_LAUNCH_MODE_ENV = "GJC_TEAM_WORKER_LAUNCH_MODE";
const GJC_TEAM_AUTO_INTERRUPT_RETRY_ENV = "GJC_TEAM_AUTO_INTERRUPT_RETRY";
const GJC_TEAM_WORKER_MCP_COMPAT_ENV = "GJC_TEAM_WORKER_MCP_COMPAT";
const GJC_LEADER_NODE_PATH_ENV = "GJC_LEADER_NODE_PATH";
const GJC_LEADER_CLI_PATH_ENV = "GJC_LEADER_CLI_PATH";
...
const TEAM_WORKER_DISABLED_GJC_MCP_SERVERS = [
	"gjc_state",
	"gjc_memory",
	"gjc_code_intel",
	"gjc_trace",
...
function tagPaneInstance(paneTarget: string, instanceId: string): void {
	...
	const result = runTmux(["set-option", "-p", "-t", target, GJC_PANE_INSTANCE_OPTION, sanitized]);
	if (!result.ok) {
...
function isHudWatchPane(pane: TmuxPaneInfo): boolean {
	...
	return /\bgjc\b.*\bhud\b.*--watch/i.test(start);
}
...
export function sleepFractionalSeconds(seconds: number, sleepImpl: (ms: number) => void = sleepSync): void {
...
function formatHudEnvAssignments(
	...
	const assignments = [
		sessionId ? `GJC_SESSION_ID=${shellQuoteSingle(sessionId)}` : "",
		`${GJC_TMUX_HUD_OWNER_ENV}=1`,
		leaderPaneId ? `${GJC_TMUX_HUD_LEADER_PANE_ENV}=${shellQuoteSingle(leaderPaneId)}` : "",
		...(typeof env.GJC_ROOT === "string" && env.GJC_ROOT.trim() !== ""
			? [`GJC_ROOT=${shellQuoteSingle(env.GJC_ROOT)}`]
			: []),
	].filter(Boolean);
...
function resolveNativeWindowsPowerShellPath(env: NodeJS.ProcessEnv = process.env): string {
	const rootCandidates = [env.SystemRoot, env.SYSTEMROOT, env.windir, env.WINDIR]
	...
	const existingCandidates = [...systemPowerShellCandidates, resolvedFromPath].filter(
		(candidate): candidate is string => Boolean(candidate && existsSync(candidate)),
	);

	return (
		existingCandidates.find(candidate => !/\s/.test(candidate)) ??
...
function buildHudResizeArgs(hudPaneId: string, heightLines: number = HUD_TMUX_TEAM_HEIGHT_LINES): string[] {
...
export function buildScheduleDelayedHudResizeArgs(
	...
	return [
		"run-shell",
		"-b",
		`sleep ${delay}; ${buildBestEffortShellCommand(buildNestedTmuxShellCommand(buildHudResizeCommand(hudPaneId, heightLines)))}`,
	];
}
...
export function shouldSourceTeamWorkerShellRc(env: NodeJS.ProcessEnv = process.env): boolean {
	return String(env.GJC_TMUX_SOURCE_SHELL_RC ?? "").trim() === "1";
}
...
function normalizeTeamWorkerCliMode(
	raw: string | undefined,
	sourceEnv: string = GJC_TEAM_WORKER_CLI_ENV,
): TeamWorkerCliMode {
...
export function resolveTeamWorkerLaunchMode(env: NodeJS.ProcessEnv = process.env): TeamWorkerLaunchMode {
	const raw = String(env[GJC_TEAM_WORKER_LAUNCH_MODE_ENV] ?? "interactive")
		.trim()
		.toLowerCase();
	if (raw === "" || raw === "interactive") return "interactive";
	if (raw === "prompt") return "prompt";
	throw new Error(
		`Invalid ${GJC_TEAM_WORKER_LAUNCH_MODE_ENV} value "${env[GJC_TEAM_WORKER_LAUNCH_MODE_ENV]}". Expected: interactive, prompt`,
	);
}

export function resolveTeamWorkerCli(launchArgs: string[] = [], env: NodeJS.ProcessEnv = process.env): TeamWorkerCli {
	const mode = normalizeTeamWorkerCliMode(env[GJC_TEAM_WORKER_CLI_ENV]);
	if (mode !== "auto") return mode;
	return resolveTeamWorkerCliFromLaunchArgs(launchArgs);
}

function resolveTeamWorkerCliFromLaunchArgs(_launchArgs: string[] = []): TeamWorkerCli {
	return "gjc";
}
...
export function resolveTeamWorkerCliPlan(
	...
	if (entries.length === 0 || entries.every(part => part.length === 0)) {
		throw new Error(
			`Invalid ${GJC_TEAM_WORKER_CLI_MAP_ENV} value "${env[GJC_TEAM_WORKER_CLI_MAP_ENV]}". ` +
				`Expected comma-separated values: auto|gjc.`,
		);
	}
	...
	if (entries.length !== 1 && entries.length !== workerCount) {
		throw new Error(
			`Invalid ${GJC_TEAM_WORKER_CLI_MAP_ENV} length ${entries.length}; ` +
	...
	return expanded.map(entry => {
		const mode = normalizeTeamWorkerCliMode(entry, GJC_TEAM_WORKER_CLI_MAP_ENV);
...
function resolveLeaderNodePath(): string {
	const envOverride = process.env[GJC_LEADER_NODE_PATH_ENV];
...
export function assertTeamWorkerCliBinaryAvailable(
	...
	throw new Error(
		`Selected team worker CLI "${workerCli}" is not available on PATH. ` +
			`Install "${workerCli}" or set ${GJC_TEAM_WORKER_CLI_ENV}=gjc.`,
	);
}

function shouldBypassDefaultSystemPrompt(env: NodeJS.ProcessEnv): boolean {
	return env[GJC_BYPASS_DEFAULT_SYSTEM_PROMPT_ENV] !== "0";
}

function buildModelInstructionsOverride(cwd: string, env: NodeJS.ProcessEnv): string {
	const filePath = translatePathForMsys(env[GJC_MODEL_INSTRUCTIONS_FILE_ENV] || join(cwd, "AGENTS.md"));
...
function shouldDisableGjcMcpForTeamWorker(env: NodeJS.ProcessEnv): boolean {
	const raw = env[GJC_TEAM_WORKER_MCP_COMPAT_ENV]?.trim().toLowerCase();
...
function appendTeamWorkerMcpDisableOverrides(args: string[], env: NodeJS.ProcessEnv): void {
	if (!shouldDisableGjcMcpForTeamWorker(env)) return;
	for (const server of TEAM_WORKER_DISABLED_GJC_MCP_SERVERS) {
...
export function buildWorkerStartupCommand(
	...
	const resolvedLeaderNodePath = processSpec.env[GJC_LEADER_NODE_PATH_ENV]?.trim() || resolveLeaderNodePath();
	...
	if (isNativeWindows()) {
		...
		const pathBootstrap = leaderNodeDir ? `$env:PATH = ${quotePowerShellArg(`${leaderNodeDir};`)} + $env:PATH` : "";
		...
		const encodedCommand = encodePowerShellCommand(
			["$ErrorActionPreference = 'Stop'", pathBootstrap, envAssignments, invocation].filter(Boolean).join("; "),
		);
	...
	const rcPrefix =
		shouldSourceTeamWorkerShellRc({ ...process.env, ...extraEnv }) && launchSpec.rcFile
...
export function buildWorkerProcessLaunchSpec(
	...
	const effectiveCliLaunchArgs =
		shouldGrantExecutionBypassForRole(workerRole) && !cliLaunchArgs.includes(CODEX_BYPASS_FLAG)
	...
	const displayTeamName =
		typeof extraEnv.GJC_TEAM_DISPLAY_NAME === "string" ? extraEnv.GJC_TEAM_DISPLAY_NAME.trim() : "";
	const publicWorkerIdentity = displayTeamName ? `${displayTeamName}/worker-${workerIndex}` : internalWorkerIdentity;
	const workerEnv: Record<string, string> = {
		GJC_TEAM_WORKER: publicWorkerIdentity,
		GJC_TEAM_INTERNAL_WORKER: internalWorkerIdentity,
		[GJC_LEADER_NODE_PATH_ENV]: resolveLeaderNodePath(),
		[GJC_LEADER_CLI_PATH_ENV]: resolvedLauncherPath,
		[GJC_TMUX_HUD_OWNER_ENV]: "1",
	};
 ...
 * GJC requires tmux, which is unavailable on native Windows.
 */
...
export function createTeamSession(
	...
	try {
		...
		const [sessionAndWindow = "", detectedLeaderPaneId = ""] = context.stdout.split(" ");
		const [sessionName, windowIndex] = (sessionAndWindow || "").split(":");
		if (!sessionName || !windowIndex || !detectedLeaderPaneId?.startsWith("%")) {
			throw new Error(`failed to parse current tmux target: ${context.stdout}`);
		}
		const teamTarget = `${sessionName}:${windowIndex}`;
		const instanceId = (process.env.GJC_SESSION_ID || "").trim();
		if (instanceId) {
			const tagResult = runTmux(["set-option", "-t", sessionName, GJC_INSTANCE_OPTION, instanceId]);
			if (!tagResult.ok) {
		...
		const gjcEntry = resolveGjcCliEntryPath();
		const canRecreateTeamHud = Boolean(gjcEntry && gjcEntry.trim() !== "");
		...
		if (canRecreateTeamHud && gjcEntry) {
			const hudCmd = `exec env ${formatHudEnvAssignments(process.env, { sessionId: instanceId, leaderPaneId })} node ${shellQuoteSingle(translatePathForMsys(gjcEntry))} hud --watch`;
				...
				if (id.startsWith("%")) {
					...
					if (isNativeWindows()) {
						...
						if (registerHook.ok) {
							...
							// still surface.
							console.warn(
								`[gjc] tmux resize hook unavailable for ${hookTarget} (${hookName}): ${registerHook.stderr}; ` +
						...
						if (registerClientAttachedHook.ok) {
							...
							console.warn(
								`[gjc] tmux client-attached resize fallback unavailable for ${hookTarget} ` +
		...
		// Opt-out: set GJC_TEAM_MOUSE=0 in the environment.
		if (process.env.GJC_TEAM_MOUSE !== "0") {
...
export function restoreStandaloneHudPane(leaderPaneId: string | null | undefined, cwd: string): string | null {
	const normalizedLeaderPaneId = normalizePaneTarget(leaderPaneId);
	if (!normalizedLeaderPaneId) return null;

	const gjcEntry = resolveGjcCliEntryPath();
	if (!gjcEntry || gjcEntry.trim() === "") return null;

	const hudCmd = `exec env ${formatHudEnvAssignments(process.env, { leaderPaneId: normalizedLeaderPaneId })} ${shellQuoteSingle(translatePathForMsys(resolveLeaderNodePath()))} ${shellQuoteSingle(translatePathForMsys(gjcEntry))} hud --watch`;
	const hudCwd = translatePathForMsys(cwd);
	const hudResult = runTmux([
		"split-window",
		"-v",
 ...
 *
 * This helper is intentionally limited to session-scoped options so GJC
...
export function enableMouseScrolling(sessionTarget: string): boolean {
	...
	// so GJC does not mutate users' global tmux style defaults. (issue #1448)
...
function paneTarget(sessionName: string, workerIndex: number, workerPaneId?: string): string {
	if (workerPaneId?.startsWith("%")) return workerPaneId;
...
export function evaluateStartupDirectTriggerSafetyCapture(
	captured: string,
	workerCli?: TeamWorkerCli,
): StartupDirectTriggerSafety {
	...
	if (workerCli === "gjc" && sharedPaneShowsCodexViewport(captured)) return { safe: true, reason: "codex_viewport" };
...
function dismissClaudeBypassPermissionsPromptIfPresent(target: string, captured: string): boolean {
	if (process.env.GJC_TEAM_AUTO_ACCEPT_BYPASS === "0") return false;
...
export async function checkWorkerStartupInjectSafety(
	...
	return scrollbackSafety === "safe" ? { safe: true, reason: "safe" } : { safe: false, reason: scrollbackSafety };
}

function resolveSendStrategyFromEnv(): "auto" | "queue" | "interrupt" {
	const raw = String(process.env.GJC_TEAM_SEND_STRATEGY || "")
		.trim()
		.toLowerCase();
...
function resolveWorkerCliFromMapForSend(
	...
	try {
		const mode = normalizeTeamWorkerCliMode(selectedRaw, GJC_TEAM_WORKER_CLI_MAP_ENV);
 ...
 * 2) per-worker GJC_TEAM_WORKER_CLI_MAP entry (worker index aware)
 * 3) global/default GJC_TEAM_WORKER_CLI behavior
 */
...
export function buildWorkerSubmitPlan(
	...
	return {
		...
		queueFirstRound: queueRequested,
		rounds: 6,
		submitKeyPressesPerRound: 2,
		allowAdaptiveRetry,
	};
}
...
function paneHasQueuedCodexSubmission(captured: string | null | undefined): boolean {
	...
	return (
		/messages to be submitted after next tool call/i.test(normalized) ||
...
async function attemptSubmitRounds(
	...
	for (let round = 0; round < rounds; round++) {
		...
		await sleep(140);
		const [captured, visibleCapture] = await Promise.all([capturePaneAsync(target), captureVisiblePaneAsync(target)]);
...
export function waitForWorkerReady(
	...
	const check = (): boolean => {
		...
		if (paneHasTrustPrompt(result.stdout)) {
			...
			// Opt-out by setting GJC_TEAM_AUTO_TRUST=0.
			if (process.env.GJC_TEAM_AUTO_TRUST !== "0") {
				sendRobustEnter();
				promptDismissed = true;
				return false;
			}
 ...
 * Opt-out: set GJC_TEAM_AUTO_TRUST=0 to disable auto-dismissal.
 */
export function dismissTrustPromptIfPresent(sessionName: string, workerIndex: number, workerPaneId?: string): boolean {
	if (process.env.GJC_TEAM_AUTO_TRUST === "0") return false;
...
export async function sendToWorker(
	...
	const allowAutoInterruptRetry = process.env[GJC_TEAM_AUTO_INTERRUPT_RETRY_ENV] !== "0";
	...
	// If you need strictness for debugging, set GJC_TEAM_STRICT_SUBMIT=1.
	const strict = process.env.GJC_TEAM_STRICT_SUBMIT === "1";
	if (strict) {
...
export function resolveSharedSessionShutdownTopology(
	...
	const resolvedLeaderPaneId =
		normalizePaneTarget(chooseTeamLeaderPaneId(panes, fallbackLeaderPaneId ?? "")) ?? fallbackLeaderPaneId;
...
export async function teardownWorkerPanes(
	...
	const perPaneGrace = killablePaneIds.length > 0 ? Math.max(100, Math.floor(graceMs / killablePaneIds.length)) : 0;
...
// List all tmux sessions matching gjc-team-* pattern
 ...
 * workers and runtime paths may message `leader-fixed` via `gjc team api`
