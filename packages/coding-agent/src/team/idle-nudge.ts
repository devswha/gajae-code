...
import { paneHasActiveTask, paneLooksReady, sendToWorker } from "./tmux-session.js";
...
/**
 * Thin wrapper to call GJC's sendToWorker by pane ID only (for NudgeTracker).
 * workerIndex=0 is a dummy — paneTarget() prefers workerPaneId when provided.
 */
async function sendToWorkerByPaneId(sessionName: string, paneId: string, message: string): Promise<boolean> {
	try {
...
export class NudgeTracker {
	 ...
	 */
	async checkAndNudge(paneIds: string[], leaderPaneId: string | undefined, sessionName: string): Promise<string[]> {
		const now = Date.now();
