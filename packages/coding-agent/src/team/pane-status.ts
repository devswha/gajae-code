...
import { teamReadTaskApproval as readTaskApproval, type teamReadConfig as readTeamConfig } from "./team-ops.js";
...
export async function readTeamPaneStatus(
	...
	const sparkshellCommands = Object.fromEntries(
		[
			leaderPaneId ? ["leader", `gjc sparkshell --tmux-pane ${leaderPaneId} --tail-lines ${tailLines}`] : null,
			hudPaneId ? ["hud", `gjc sparkshell --tmux-pane ${hudPaneId} --tail-lines ${tailLines}`] : null,
			...Object.entries(workerPanes).map(
				([workerName, paneId]) =>
					[workerName, `gjc sparkshell --tmux-pane ${paneId} --tail-lines ${tailLines}`] as const,
			),
		].filter((entry): entry is [string, string] => entry !== null),
	);
	...
	const recommendedInspectTargets = [
		...
		(workerName, index, values) => Object.hasOwn(workerPanes, workerName) && values.indexOf(workerName) === index,
	);
	...
	const taskCreatedAtById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.created_at ?? null] as const),
	);
	const taskCompletedAtById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.completed_at ?? null] as const),
	);
	const taskDependsOnById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.depends_on ?? task.blocked_by ?? []] as const),
	);
	const taskClaimPresentById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.claim != null] as const),
	);
	const taskClaimOwnerById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.claim?.owner ?? null] as const),
	);
	const taskClaimTokenById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.claim?.token ?? null] as const),
	);
	const taskClaimLeaseById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.claim?.leased_until ?? null] as const),
	);
	const taskRequiresCodeChangeById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.requires_code_change ?? null] as const),
	);
	...
	const recommendedInspectTaskClaimLockPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => {
			...
			return [
				target,
				taskId && snapshot?.teamName
					? join(cwd, ".gjc", "state", "team", snapshot.teamName, "claims", `task-${taskId}.lock`)
					: null,
			];
		}),
	);
	...
	const taskBlockedById = new Map(
		(snapshot?.tasks.items ?? []).map(task => [task.id, task.blocked_by ?? []] as const),
	);
	...
	for (const taskId of new Set(
		Object.values(recommendedInspectTasks).filter(
			(value): value is string => typeof value === "string" && value.length > 0,
		),
	)) {
		approvalRecordByTaskId.set(
			taskId,
			snapshot?.teamName ? await readTaskApproval(snapshot.teamName, taskId, cwd) : null,
		);
	}
	...
	const recommendedInspectTaskPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => {
			...
			return [
				target,
				taskId && snapshot?.teamName
					? join(cwd, ".gjc", "state", "team", snapshot.teamName, "tasks", `task-${taskId}.json`)
					: null,
			];
		}),
	);
	const recommendedInspectApprovalPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => {
			...
			return [
				target,
				taskId && snapshot?.teamName
					? join(cwd, ".gjc", "state", "team", snapshot.teamName, "approvals", `task-${taskId}.json`)
					: null,
			];
		}),
	);
	const recommendedInspectWorkerStateDirs = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target) : null,
		]),
	);
	const recommendedInspectWorkerStatusPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName
				? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target, "status.json")
				: null,
		]),
	);
	const recommendedInspectWorkerHeartbeatPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName
				? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target, "heartbeat.json")
				: null,
		]),
	);
	const recommendedInspectWorkerIdentityPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName
				? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target, "identity.json")
				: null,
		]),
	);
	const recommendedInspectWorkerInboxPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName
				? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target, "inbox.md")
				: null,
		]),
	);
	const recommendedInspectWorkerMailboxPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "mailbox", `${target}.json`) : null,
		]),
	);
	const recommendedInspectWorkerShutdownRequestPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName
				? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target, "shutdown-request.json")
				: null,
		]),
	);
	const recommendedInspectWorkerShutdownAckPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName
				? join(cwd, ".gjc", "state", "team", snapshot.teamName, "workers", target, "shutdown-ack.json")
				: null,
		]),
	);
	const recommendedInspectTeamConfigPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "config.json") : null,
		]),
	);
	const recommendedInspectTeamManifestPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "manifest.v2.json") : null,
		]),
	);
	const recommendedInspectTeamEventsPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "events", "events.ndjson") : null,
		]),
	);
	const recommendedInspectTeamDispatchPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "dispatch", "requests.json") : null,
		]),
	);
	const recommendedInspectTeamDirPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName) : null,
		]),
	);
	const recommendedInspectTeamPhasePaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "phase.json") : null,
		]),
	);
	const recommendedInspectTeamMonitorSnapshotPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "monitor-snapshot.json") : null,
		]),
	);
	const recommendedInspectTeamSummarySnapshotPaths = Object.fromEntries(
		recommendedInspectTargets.map(target => [
			target,
			snapshot?.teamName ? join(cwd, ".gjc", "state", "team", snapshot.teamName, "summary-snapshot.json") : null,
		]),
	);
	...
	const recommendedInspectCommand =
		recommendedInspectTargets.length > 0 ? (sparkshellCommands[recommendedInspectTargets[0]!] ?? null) : null;
	...
	const recommendedInspectSummary =
		recommendedInspectTargets.length > 0
			? [
					`target=${recommendedInspectTargets[0]}`,
					recommendedInspectPanes[recommendedInspectTargets[0]!]
						? `pane=${recommendedInspectPanes[recommendedInspectTargets[0]!]}`
						: "",
					recommendedInspectClis[recommendedInspectTargets[0]!]
						? `cli=${recommendedInspectClis[recommendedInspectTargets[0]!]}`
						: "",
					recommendedInspectRoles[recommendedInspectTargets[0]!]
						? `role=${recommendedInspectRoles[recommendedInspectTargets[0]!]}`
						: "",
					typeof recommendedInspectAlive[recommendedInspectTargets[0]!] === "boolean"
						? `alive=${recommendedInspectAlive[recommendedInspectTargets[0]!]}`
						: "",
					typeof recommendedInspectTurnCounts[recommendedInspectTargets[0]!] === "number"
						? `turn_count=${recommendedInspectTurnCounts[recommendedInspectTargets[0]!]}`
						: "",
					typeof recommendedInspectTurnsWithoutProgress[recommendedInspectTargets[0]!] === "number"
						...
						: "",
					recommendedInspectReasons[recommendedInspectTargets[0]!]
						? `reason=${recommendedInspectReasons[recommendedInspectTargets[0]!]}`
						: "",
					recommendedInspectStates[recommendedInspectTargets[0]!]
						? `state=${recommendedInspectStates[recommendedInspectTargets[0]!]}`
						: "",
					recommendedInspectTasks[recommendedInspectTargets[0]!]
						? `task=${recommendedInspectTasks[recommendedInspectTargets[0]!]}`
						: "",
					recommendedInspectSubjects[recommendedInspectTargets[0]!]
						? `subject=${recommendedInspectSubjects[recommendedInspectTargets[0]!]}`
						: "",
	...
	return {
		...
		sparkshell_hint:
			Object.keys(workerPanes).length > 0 ? "gjc sparkshell --tmux-pane <pane-id> --tail-lines 400" : null,
