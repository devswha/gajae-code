 ...
 * Gated behind the GJC_TEAM_SCALING_ENABLED environment variable.
 *
...
import { type DispatchOutcome, queueInboxInstruction, waitForDispatchReceipt } from "./mcp-comm.js";
import {
...
import { readPersistedTeamUltragoalContext, renderLeaderOwnedUltragoalContextSection } from "./ultragoal-context.js";
import {
...
const GJC_TEAM_SCALING_ENABLED_ENV = "GJC_TEAM_SCALING_ENABLED";
...
export function isScalingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
	const raw = env[GJC_TEAM_SCALING_ENABLED_ENV];
	if (!raw) return false;
...
function assertScalingEnabled(env: NodeJS.ProcessEnv = process.env): void {
	if (!isScalingEnabled(env)) {
		throw new Error(`Dynamic scaling is disabled. Set ${GJC_TEAM_SCALING_ENABLED_ENV}=1 to enable.`);
	}
}
...
function resolveLegacyScaledTeamWorktreeMode(
	config: Pick<TeamConfig, "name" | "workspace_mode" | "worktree_mode" | "workers">,
): WorktreeMode {
	if (config.worktree_mode) return config.worktree_mode;
	if (config.workspace_mode !== "worktree") return { enabled: false };

	const workersWithMetadata = config.workers.filter(
		worker => worker.worktree_path || worker.worktree_branch || typeof worker.worktree_detached === "boolean",
	);
...
export async function scaleUp(
	...
	return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleUpResult | ScaleError> => {
		...
		const approvedExecutionGate = resolveScaleUpApprovedExecutionGate(sanitized, approvedExecutionState);
		...
		const rollbackScaleUp = async (
			...
			for (const w of addedWorkers) {
				...
				try {
					if (w.pane_id) {
						execFileSync("tmux", ["kill-pane", "-t", w.pane_id], { stdio: "pipe", windowsHide: true });
					}
				} catch {}
				if (w.worktree_path) {
			...
			if (context.paneId) {
				try {
					execFileSync("tmux", ["kill-pane", "-t", context.paneId], { stdio: "pipe", windowsHide: true });
				} catch {}
			}

			for (const taskId of createdTaskIds) {
				await rm(join(leaderCwd, ".gjc", "state", "team", sanitized, "tasks", `task-${taskId}.json`), {
					force: true,
				}).catch(() => {});
			}
		...
		for (const task of tasks) {
			const createdTask = await createStateTask(
				sanitized,
				{
		...
		for (let i = 0; i < count; i++) {
			...
			const workerDirPath = join(leaderCwd, ".gjc", "state", "team", sanitized, "workers", workerName);
			...
			const workerTaskRoles = persistedTasks
			...
			if (uniqueTaskRoles.size > 1) {
				console.log(
					`[gjc:scaling] ${workerName}: mixed task roles [${[...uniqueTaskRoles].join(", ")}], falling back to ${agentType}`,
				);
			}
			...
			const rolePromptContent = rawRolePromptContent
				...
				: null;
			const teamInstructionsPath = join(leaderCwd, ".gjc", "state", "team", sanitized, "worker-agents.md");
			...
			const extraEnv: Record<string, string> = {
				GJC_TEAM_STATE_ROOT: teamStateRoot,
				GJC_TEAM_LEADER_CWD: leaderCwd,
				GJC_MODEL_INSTRUCTIONS_FILE: instructionsFilePath,
			};
			if (workerWorkspace) {
				extraEnv.GJC_TEAM_WORKTREE_PATH = workerWorkspace.worktreePath;
				if (workerWorkspace.branchName) {
					extraEnv.GJC_TEAM_WORKTREE_BRANCH = workerWorkspace.branchName;
				}
				extraEnv.GJC_TEAM_WORKTREE_DETACHED = workerWorkspace.detached ? "1" : "0";
			}
			...
			const skipReadyWait = env.GJC_TEAM_SKIP_READY_WAIT === "1";
			if (!skipReadyWait) {
				const ready = waitForWorkerReady(sessionName, workerIndex, readyTimeoutMs, paneId);
				if (!ready) {
					console.log(`[gjc:scaling] Warning: worker ${workerName} did not become ready within timeout`);
				}
			}
			...
			if (dispatchPolicy.dispatch_mode === "hook_preferred_with_fallback" && queued.request_id) {
				...
				if (receipt && (receipt.status === "notified" || receipt.status === "delivered")) {
					...
					const fallback = await notifyWorkerPaneOutcome(
						sessionName,
						workerIndex,
		...
		for (const w of targetWorkers) {
			if (w.worktree_path) {
				await removeWorkerWorktreeRootAgentsFile(
					sanitized,
					w.name,
					w.team_state_root ?? config.team_state_root ?? resolveCanonicalTeamStateRoot(leaderCwd),
					w.worktree_path,
				).catch(() => {});
			}
