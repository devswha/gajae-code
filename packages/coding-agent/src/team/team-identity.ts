...
export function resolveTeamIdentityScope(env: NodeJS.ProcessEnv = process.env): TeamIdentityScope {
	const sessionId = (env.GJC_SESSION_ID || env.CODEX_SESSION_ID || env.SESSION_ID || "").trim();
	if (sessionId) {
		return {
			sessionId,
			paneId: (env.TMUX_PANE || "").trim(),
			tmuxTarget: (env.TMUX || "").trim(),
			runId: "",
			source: "env-session",
		};
	}
...
export function buildInternalTeamName(displayName: string, scope: TeamIdentityScope): string {
	...
	const identity =
		scope.sessionId || (scope.tmuxTarget || scope.paneId ? `${scope.tmuxTarget}:${scope.paneId}` : "") || scope.runId;
...
function candidateFromDir(root: string, teamName: string): TeamLookupCandidate | null {
	...
	const leader =
		source.leader && typeof source.leader === "object" && !Array.isArray(source.leader)
	...
	const displayName =
		...
		str(source.requested_name) ||
		str(config?.display_name) ||
		str(config?.requested_name) ||
		teamName;
	return {
		teamName,
		displayName,
...
function teamLookupRoots(cwd: string, env: NodeJS.ProcessEnv = process.env): string[] {
	...
	const explicit = typeof env.GJC_TEAM_STATE_ROOT === "string" ? env.GJC_TEAM_STATE_ROOT : "";
	addStateRoot(explicit);
	addStateRoot(join(resolve(cwd), ".gjc", "state"));
	return roots;
}
...
function matchingCurrentLeader(candidate: TeamLookupCandidate, scope: TeamIdentityScope): boolean {
	return (
		(Boolean(scope.sessionId) && candidate.leaderSessionId === scope.sessionId) ||
...
export function resolveTeamNameForCurrentContext(
	inputName: string,
	cwd: string,
