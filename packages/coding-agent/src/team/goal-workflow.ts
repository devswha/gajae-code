...
export function buildTeamWorkerGoalInstruction(
	...
	const objective =
		options.objective ??
		`Complete assigned GJC team task${taskIds.length === 1 ? "" : "s"} ${taskIds.join(", ")} for ${teamName} with verified evidence, preserving leader-owned audit.`;
...
export function renderTeamWorkerGoalInstruction(instruction: TeamWorkerGoalInstruction | undefined): string {
...
- Claim each task with \`gjc team api claim-task\` before editing; use the task file claim/status as the current assignment record.
- Record completion evidence through \`gjc team api transition-task-status\`; leader audit owns aggregate team completion.
...
3. Do not claim GJC shell commands mutated Codex goal state; shell/team APIs persist only GJC artifacts and task state.
