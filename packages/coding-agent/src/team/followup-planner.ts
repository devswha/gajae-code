...
const SHORT_RALPH_FOLLOWUP_PATTERNS: RegExp[] = [/^ralph$/i, /^ralph\s+please$/i];
...
export function isApprovedExecutionFollowupShortcut(
	...
	return mode === "team" ? isShortTeamFollowupRequest(text) : isShortRalphFollowupRequest(text);
}
...
function buildLaunchHints(
	...
	if (mode === "team") {
		return {
			shellCommand: `gjc team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
			skillCommand: `$team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
			rationale:
				"Launch team directly when coordinated parallel delivery plus built-in verification lanes are sufficient without a separate linked Ralph launch.",
		};
	}

	return {
		shellCommand: `gjc ralph ${toQuotedCliArg(task)}`,
		skillCommand: `$ralph ${toQuotedCliArg(task)}`,
		rationale:
			"Launch Ralph directly when one persistent implementation + verification loop is sufficient without team coordination overhead.",
	};
}
...
function buildVerificationPlan(
	...
	if (mode === "team") {
		...
		return {
			...
			checkpoints: [
				"Launch via `gjc team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.",
...
function pickSpecialistRole(
	...
	const wantsExplore =
		(/\b(?:check|find|inspect|locate|look up|lookup|map|review|search|trace|understand|which files?|where(?:\s+is|\s+are)?)\b/.test(
			normalizedTask,
		) &&
			/\b(?:file|files|symbol|symbols|repo|repository|codebase|path|paths|usage|usages|relationship|relationships|implementation|local|call sites?|integration points?)\b/.test(
				normalizedTask,
			)) ||
		/\b(?:call sites?|current(?:ly)? use|how we use|integration points?|our usage|where we use)\b/.test(
			normalizedTask,
		);
	const wantsDependencyExpert =
		/\b(?:dependency|dependencies|package|packages|sdk|sdks|library|libraries|framework|frameworks|npm|pypi|license|maintenance|migration path|download stats?)\b/.test(
			normalizedTask,
		) &&
		/\b(?:adopt|assess|choose|compare|evaluate|recommend|replace|risk|select|swap|upgrade)\b/.test(normalizedTask);
	const wantsResearcher =
		/\b(?:official docs?|upstream docs?|vendor docs?|reference|references|api docs?|release notes?|version(?:ing)?|compatib(?:ility|le)|research)\b/.test(
			normalizedTask,
		) ||
		(/\b(?:api|framework|frameworks|library|libraries|sdk|sdks|vendor)\b/.test(normalizedTask) &&
			/\b(?:best way|behavior|example|examples|feature|features?|how to use|in the wild|parameter|parameters|usage|what does|why does)\b/.test(
				normalizedTask,
			));
	...
	if (mode === "team") {
		...
		if (workerCount >= 4) {
			...
			mergeAllocation(
				...
				specialistRole,
				workerCount - 3,
				"parallel specialist follow-up capacity",
				options.codexHomeOverride,
			);
		}
	}
