...
async function readTeamWorktreePaths(cwd: string, teamName: string): Promise<string[]> {
	const teamRoot = join(cwd, ".gjc", "state", "team", teamName);
...
async function readTeamNudgeProgressMs(cwd: string, teamName: string): Promise<number> {
	const nudgeState = await readJsonIfExists(join(cwd, ".gjc", "state", "team-leader-nudge.json"));
...
async function readCurrentTaskBaselineMs(worktreePath: string): Promise<number> {
	return await statMsIfExists(join(worktreePath, ".gjc", "state", "current-task-baseline.json"));
}

export async function readLatestTeamProgressEvidenceMs(cwd: string, teamName: string): Promise<number> {
