 ...
 * to stdout.
 *
 * Spawned by GJC team orchestration entrypoints when a background team run starts.
 */
...
import type { StaleTeamSummary, TeamRuntime, TeamShutdownSummary, TeamSnapshot } from "./runtime.js";
import { monitorTeam, shutdownTeam, startTeam } from "./runtime.js";
...
async function promptStaleCleanup(summary: StaleTeamSummary): Promise<boolean> {
	process.stderr.write(
		`\n[gjc] Stale artifacts from previous team "${summary.teamName}":\n` +
			`  Worktrees: ${summary.worktreePaths.join(", ")}\n` +
			`  State dir: ${summary.statePath}\n` +
			(summary.hasDirtyWorktrees ? "  ⚠ Some worktrees have uncommitted changes that will be lost.\n" : "") +
...
async function writePanesFile(jobId: string | undefined, paneIds: string[], leaderPaneId: string): Promise<void> {
	const gjcJobsDir = process.env.GJC_JOBS_DIR;
	if (!jobId || !gjcJobsDir) return;

	const panesPath = join(gjcJobsDir, `${jobId}-panes.json`);
...
export function buildTerminalCliResult(
	...
	return {
		...
		notice:
			...
			`Inspect with "gjc team status ${teamName} --json" or "gjc team api read-stall-state --input '{"team_name":"${teamName}"}' --json". ` +
			`Run "gjc team shutdown ${teamName}" (or --force after state capture) when explicit cleanup is desired.\n`,
	};
}
...
export function normalizeAgentTypes(raw: string[], workerCount: number): TeamWorkerProvider[] {
	...
	const invalid = providers.filter(entry => entry !== "gjc" && entry !== "auto");
	if (invalid.length > 0) {
		throw new Error(`Invalid agentTypes entries: ${invalid.join(", ")}. Expected gjc.`);
	}
...
export function resolveRuntimeCliProviderMap(raw: string[] | undefined, workerCount: number): string | null {
...
async function main(): Promise<void> {
	...
	async function doShutdown(status: "completed" | "failed"): Promise<void> {
		...
		if (shutdownSummary?.commitHygieneArtifacts) {
			process.stderr.write(
				`[runtime-cli] commit_hygiene_context_json=${shutdownSummary.commitHygieneArtifacts.jsonPath}\n`,
			);
			process.stderr.write(
				`[runtime-cli] commit_hygiene_context_md=${shutdownSummary.commitHygieneArtifacts.markdownPath}\n`,
			);
		}
	...
	// Start the team — GJC's startTeam takes individual parameters
	...
	try {
		...
		const previousCliMap = process.env.GJC_TEAM_WORKER_CLI_MAP;
		try {
			if (providerMap) {
				process.env.GJC_TEAM_WORKER_CLI_MAP = providerMap;
			}
			runtime = await startTeam(teamName, task, agentType, workerCount, tasks, cwd, {
				codexHomeOverride: resolveCodexHomeForLaunch(cwd, process.env),
				confirmStaleCleanup: promptStaleCleanup,
				...(Object.hasOwn(input, "approvedExecution")
			...
			if (providerMap) {
				if (typeof previousCliMap === "string") process.env.GJC_TEAM_WORKER_CLI_MAP = previousCliMap;
				else delete process.env.GJC_TEAM_WORKER_CLI_MAP;
			}
		}
	} catch (err) {
	...
	// Persist pane IDs when a background launcher provides an GJC job ID.
	const jobId = process.env.GJC_JOB_ID;
	try {
...
const shouldAutoStart = process.env.GJC_RUNTIME_CLI_DISABLE_AUTO_START !== "1";
