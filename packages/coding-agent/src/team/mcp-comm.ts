...
function isLeaderPaneMissingMailboxPersistedOutcome(request: TeamDispatchRequest, outcome: DispatchOutcome): boolean {
	return (
		request.to_worker === "leader-fixed" && outcome.ok && outcome.reason === "leader_pane_missing_mailbox_persisted"
	);
}
...
async function logDispatchOutcome(params: {
	...
	const { cwd, teamName, source, requestId, messageId, toWorker, dispatchKind, outcome, intent, transportPreference } =
		params;
	const result = outcome.ok ? (outcome.reason === "queued_for_hook_dispatch" ? "queued" : "ok") : "failed";
...
export async function queueDirectMailboxMessage(params: QueueDirectMessageParams): Promise<DispatchOutcome> {
	...
	if (message.notified_at && !message.delivered_at) {
		const outcome = {
			ok: true,
			transport:
				params.toWorker === "leader-fixed" ? "mailbox" : fallbackTransportForPreference(params.transportPreference),
