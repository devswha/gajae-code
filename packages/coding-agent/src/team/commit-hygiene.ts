...
function projectRootFromGjcStateRoot(stateRoot: string | null, cwd: string): string | null {
	if (!stateRoot) return null;
	const resolvedStateRoot = resolve(cwd, stateRoot);
	const gjcDir = dirname(resolvedStateRoot);
	if (basename(resolvedStateRoot) !== "state" || basename(gjcDir) !== ".gjc") return null;
	return dirname(gjcDir);
}
...
export function resolveTeamCommitHygieneArtifactCwd(
	...
	const derivedFromStateRoot = projectRootFromGjcStateRoot(safePath(config?.team_state_root), cwd);
...
function commitHygieneReportsDir(cwd: string): string {
	return join(resolve(cwd), ".gjc", "reports", "team-commit-hygiene");
}
...
export async function readTeamCommitHygieneLedger(teamName: string, cwd: string): Promise<TeamCommitHygieneLedger> {
	...
	try {
		...
		return {
			version: 1,
			team_name:
				typeof parsed.team_name === "string" && parsed.team_name.trim() !== "" ? parsed.team_name : teamName,
...
const TEAM_COMMIT_HYGIENE_VOCABULARY: TeamCommitHygieneVocabulary = {
	operational_commit_kinds: [
		{
			...
			description:
				"A worker-local checkpoint commit created by the team runtime to preserve dirty worktree changes.",
		},
		{
			value: "integration_merge",
			label: "integration merge",
			description:
				"A leader-side runtime merge commit that integrates a worker branch or checkpoint into the team branch.",
		},
		{
			value: "integration_cherry_pick",
			label: "integration cherry-pick",
			description:
				"A leader-side runtime cherry-pick used when the normal worker merge path cannot be used cleanly.",
		},
		{
			value: "cross_rebase",
			label: "cross-rebase",
		...
		},
		{
			...
			description:
				"A leader-side integration attempt recorded for auditability even when it does not create a final semantic commit.",
		},
		{
			value: "shutdown_checkpoint",
			label: "shutdown checkpoint",
			description:
				"A shutdown-time checkpoint commit that preserves remaining worker worktree changes before cleanup.",
		},
		{
			value: "shutdown_merge",
			label: "shutdown merge",
			description:
				"A shutdown-time runtime merge that preserves worker changes on the leader branch before teardown.",
		},
	],
	operational_commit_statuses: [
		...
		},
		{
			...
			label: "no-op",
			description:
				"The runtime operation was unnecessary because there was no relevant change to preserve or integrate.",
		},
		{
			value: "conflict",
			label: "conflict",
		...
		},
		{
			...
			label: "skipped",
			description:
				"The runtime intentionally skipped the operation because prerequisites or safety checks were not met.",
		},
	],
};
...
export function buildTeamCommitHygieneContext(params: {
	...
	const recommendedNextSteps = [
		...
		"Derive semantic commit boundaries from completed task subjects, code diffs, and shutdown reports rather than from gjc(team) operational commit subjects.",
...
export function renderTeamCommitHygieneMarkdown(context: TeamCommitHygieneContext): string {
	return [
		...
		"",
		"1. Treat `gjc(team): ...` runtime commits as temporary scaffolding, not as the final PR history.",
