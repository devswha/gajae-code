 ...
 *
 * See: https://github.com/Yeachan-Heo/gajae-code/issues/1100
 */
...
function stateDirToProjectRoot(stateDir: string): string {
	...
	while (current !== dirname(current)) {
		if (basename(current) === ".gjc") return dirname(current);
...
export function leaderRuntimeActivityPath(cwd: string): string {
	return join(gjcStateDir(cwd), "leader-runtime-activity.json");
}
...
export async function recordLeaderRuntimeActivity(
	...
	const existing: LeaderRuntimeActivityDoc =
		existingRaw && typeof existingRaw === "object" ? (existingRaw as LeaderRuntimeActivityDoc) : {};
