...
import { gjcLogsDir } from "../utils/paths.js";
...
export async function appendTeamDeliveryLogForCwd(cwd: string, event: TeamDeliveryLogEvent): Promise<void> {
	const primaryLogsDir = gjcLogsDir(cwd);
	await appendTeamDeliveryLog(primaryLogsDir, event);

	const localLogsDir = join(cwd, ".gjc", "logs");
	if (localLogsDir !== primaryLogsDir && existsSync(join(cwd, ".gjc", "state", "team"))) {
