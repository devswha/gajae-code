...
export function normalizeApprovedTeamExecutionBinding(value: unknown): ApprovedTeamExecutionBinding | null {
	...
	const command = typeof binding.command === "string" ? binding.command.trim() : "";
...
function renderApprovedRepositoryContextSummary(summary: ApprovedRepositoryContextSummary): string[] {
	const lines = [
...
function readApprovedHintSourceText(approvedHint: ApprovedExecutionLaunchHint): string {
	try {
...
export function buildUltragoalCheckpointGuidance(
	...
	const detectionText = [approvedHint.task, approvedHint.command ?? "", sourceText].join("\n");
	if (!/ultragoal|\.gjc\/ultragoal/i.test(detectionText)) {
		return null;
	}

	const goalId = detectUltragoalId(detectionText);
	const goalIdDisplay = goalId ?? "<read .gjc/ultragoal/goals.json first>";
	return {
		...
		checkpoint_command_template:
			"<leader must read verified .gjc/ultragoal/goals.json context before constructing checkpoint command>",
		final_checkpoint_command_template:
			"<leader must read verified .gjc/ultragoal/goals.json context and pass final quality gates before constructing checkpoint command>",
		evidence_requirements: [
			"team tasks are terminal",
			"verification passed",
			goalId ? `evidence mentions ${goalId}` : "leader resolved the active goal ID from .gjc/ultragoal/goals.json",
...
export function renderLeaderOwnedUltragoalContext(guidance: UltragoalCheckpointGuidance | null | undefined): string[] {
	if (!guidance) return [];
	return [
		"",
		"- Approved-plan Ultragoal hint:",
		"  - source: approved Team handoff text; leader must verify `.gjc/ultragoal/goals.json` before checkpointing.",
...
function approvedTeamExecutionBindingPath(teamName: string, cwd: string, teamStateRoot?: string | null): string {
