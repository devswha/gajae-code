...
const LOW_COMPLEXITY_AGENT_TYPES = new Set(["explore", "explorer", "style-reviewer"]);
...
export function resolveAgentReasoningEffort(
	...
	return (
		normalizeOptionalReasoning(getAgentReasoningOverride(agentType, codexHomeOverride)) ??
		normalizeOptionalReasoning(getAgent(agentType)?.reasoningEffort)
	);
}

export function resolveAgentDefaultModel(agentType?: string, codexHomeOverride?: string): string | undefined {
	if (typeof agentType !== "string" || agentType.trim() === "") return undefined;
