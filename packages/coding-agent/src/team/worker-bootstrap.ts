...
import { renderTeamWorkerGoalInstruction, type TeamWorkerGoalInstruction } from "./goal-workflow.js";
...
import workerSkillAddon from "./worker-skill.md" with { type: "text" };

const TEAM_OVERLAY_START = "<!-- GJC:TEAM:WORKER:START -->";
const TEAM_OVERLAY_END = "<!-- GJC:TEAM:WORKER:END -->";
const SKILL_REFERENCE_PATTERN = /\/skills\/([^/\s`]+)\/SKILL\.md\b/g;
const AGENTS_LOCK_PATH = [".gjc", "state", "agents-md.lock"];
const LOCK_OWNER_FILE = "owner.json";
const LOCK_TIMEOUT_MS = 5000;
const LOCK_POLL_INTERVAL_MS = 100;
const LOCK_STALE_MS = 30_000;
function renderWorkerSkillAddon(): string {
	return `## Team Worker Protocol Add-on

${workerSkillAddon.trim()}`;
}
...
function buildWorkerRootAgentsBackupPath(
	...
	const gitPath = tryReadGitValue(worktreePath, ["rev-parse", "--git-path", "gjc/root-agents-backup.json"]);
	return gitPath ? gitPath : join(teamStateRoot, "team", teamName, "workers", workerName, "root-agents-backup.json");
}

export function generateWorkerRootAgentsContent(options: WorkerRootAgentsOptions): string {
	return `# Team Worker Runtime Instructions

This file is generated for a live GJC team worker run and is disposable.
...
## Protocol
1. Follow the Team Worker Protocol Add-on in this generated instruction file.
2. Read your inbox at \`${options.teamStateRoot}/team/${options.teamName}/workers/${options.workerName}/inbox.md\`.
3. Send startup ACK before task work:

   \`gjc team api send-message --input "{"team_name":"${options.teamName}","from_worker":"${options.workerName}","to_worker":"leader-fixed","body":"ACK: ${options.workerName} initialized"}" --json\`

4. Resolve canonical team state root in this order: \`GJC_TEAM_STATE_ROOT\` env -> worker identity \`team_state_root\` -> config/manifest \`team_state_root\` -> local cwd fallback.
...
6. Use claim-safe lifecycle APIs only:
   - \`gjc team api claim-task --json\`
   - \`gjc team api transition-task-status --json\`
   - \`gjc team api release-task-claim --json\` only for rollback to pending
7. Use mailbox delivery flow:
   - \`gjc team api mailbox-list --input "{"team_name":"${options.teamName}","worker":"${options.workerName}"}" --json\`
   - \`gjc team api mailbox-mark-delivered --input "{"team_name":"${options.teamName}","worker":"${options.workerName}","message_id":"<MESSAGE_ID>"}" --json\`
8. Preserve leader steering via inbox/mailbox nudges; task payload stays in inbox/task JSON, not this file.
9. Do not pass \`workingDirectory\` to legacy team_* MCP tools; use \`gjc team api\` CLI interop.

${renderWorkerSkillAddon()}
## Message Protocol
...
async function ensureGitInfoExcludePattern(worktreePath: string, pattern: string): Promise<void> {
	const excludePath = tryReadGitValue(worktreePath, ["rev-parse", "--git-path", "info/exclude"]);
...
export async function writeWorkerWorktreeRootAgentsFile(options: WorkerRootAgentsOptions): Promise<string> {
	...
	const previousContent = existed ? await readFile(agentsPath, "utf-8") : undefined;
...
export async function removeWorkerWorktreeRootAgentsFile(
	...
	try {
		backup = JSON.parse(await readFile(backupPath, "utf-8")) as WorkerRootAgentsBackup;
	} catch {
		backup = null;
	}
...
## Protocol
1. Follow the Team Worker Protocol Add-on embedded below; do not look for a visible \`worker\` skill.
2. Read your inbox file at the path provided in your first instruction.
3. Send an ACK to the lead using CLI interop \`gjc team api send-message --json\` (to_worker="leader-fixed") once initialized.
...
7. Request a claim via CLI interop (\`gjc team api claim-task --json\`); do not directly set lifecycle fields in the task file
...
10. On completion/failure, use lifecycle transition APIs:
   - \`gjc team api transition-task-status --json\` with from \`"in_progress"\` to \`"completed"\` or \`"failed"\`
   - Include \`result\` (for completed) or \`error\` (for failed) in the transition patch
11. Use \`gjc team api release-task-claim --json\` only for rollback/requeue to \`pending\` (not for completion)
...
15. For legacy team_* MCP tools (hard-deprecated), switch to \`gjc team api\` CLI interop; do not pass workingDirectory unless the lead explicitly tells you to

${renderWorkerSkillAddon()}

## Message Protocol
When calling \`gjc team api send-message\`, you MUST always include:
...
Keep the body short and deterministic so all GJC workers behave consistently.

Example:
gjc team api send-message --input "{"team_name":"${teamName}","from_worker":"<your-worker-name>","to_worker":"leader-fixed","body":"ACK: <your-worker-name> initialized"}" --json
...
1. Read: \`gjc team api mailbox-list --input "{"team_name":"${teamName}","worker":"<your-worker-name>"}" --json\`
2. Mark delivered: \`gjc team api mailbox-mark-delivered --input "{"team_name":"${teamName}","worker":"<your-worker-name>","message_id":"<MESSAGE_ID>"}" --json\`
...
- You may spawn GJC-native subagents when parallel execution improves throughput.
...
function dropShadowedSkillReferenceLines(content: string, shadowedSkillNames: ReadonlySet<string>): string {
	...
	const keptLines = lines.filter(line => {
		...
		for (match = SKILL_REFERENCE_PATTERN.exec(line); match !== null; match = SKILL_REFERENCE_PATTERN.exec(line)) {
 ...
 * AGENTS.md, the project's AGENTS.md (if any), and the source-bundled
 * worker add-on. This avoids mutating the source AGENTS.md files directly.
 *
 * Returns the absolute path to the composed file.
 */
export async function writeTeamWorkerInstructionsFile(teamName: string, cwd: string, overlay: string): Promise<string> {
	...
	const projectSkillNames = new Set(
		installedSkills.filter(skill => skill.scope === "project").map(skill => skill.name),
	);
	...
	const outPath = join(cwd, ".gjc", "state", "team", teamName, "worker-agents.md");
...
${roleOverlay}`
	...
	const outPath = join(cwd, ".gjc", "state", "team", teamName, "workers", workerName, "AGENTS.md");
	await mkdir(dirname(outPath), { recursive: true });
	await writeFile(outPath, composed);
	return outPath;
}
 ...
 */
export async function removeTeamWorkerInstructionsFile(teamName: string, cwd: string): Promise<void> {
	const outPath = join(cwd, ".gjc", "state", "team", teamName, "worker-agents.md");
...
function renderDelegationContract(task: TeamTask): string {
	...
	const candidates = (plan.subtask_candidates ?? []).map(candidate => `- ${candidate}`).join("\n");
...
- Before doing more than ${threshold} serial repo-search/read commands, spawn up to ${maxParallel} GJC-native subagents using model ${childModel}, wait for them, then integrate their findings before continuing.
...
- Include exactly one of these lines in the task completion \`result\` passed to \`gjc team api transition-task-status\`:
 ...
 * This is written to .gjc/state/team/{team}/workers/{worker}/inbox.md by the lead.
 */
...
1. Follow the Team Worker Protocol Add-on already embedded in your generated worker instructions; do not look for a visible \`worker\` skill.
...
7. Request a claim via CLI interop (\`gjc team api claim-task --json\`) to claim it
8. Complete the work described in the task
9. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
10. Complete/fail it via lifecycle transition API (\`gjc team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
11. Use \`gjc team api release-task-claim --json\` only for rollback to \`pending\`
...
14. For legacy team_* MCP tools (hard-deprecated), use \`gjc team api\`; do not pass \`workingDirectory\` unless the lead explicitly asks (if resolution fails, use leader cwd: \`${leaderCwd}\`)

## Mailbox Delivery Protocol (Required)
When you are notified about mailbox messages, always follow this exact flow:

1. List mailbox:
   \`gjc team api mailbox-list --input "{"team_name":"${teamName}","worker":"${workerName}"}" --json\`
2. For each undelivered message, mark delivery:
   \`gjc team api mailbox-mark-delivered --input "{"team_name":"${teamName}","worker":"${workerName}","message_id":"<MESSAGE_ID>"}" --json\`

Use terse ACK bodies (single line) for consistent parsing across GJC workers.
...
## Message Protocol
When using \`gjc team api send-message\`, ALWAYS include from_worker with YOUR worker name:
- from_worker: "${workerName}"
- to_worker: "leader-fixed" (for leader) or "worker-N" (for peers)

Example: gjc team api send-message --input "{"team_name":"${teamName}","from_worker":"${workerName}","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
...
export function generateTaskAssignmentInbox(
	...
	const options = typeof taskDescriptionArg === "string" ? optionsArg : (taskDescriptionArg ?? optionsArg);
	const task =
	...
	const workerGoalSection =
		typeof taskOrId === "string"
			...
			: renderTeamWorkerGoalInstruction({
					teamName,
					workerName,
					objective: `Complete assigned GJC team task ${taskOrId.id} with verified evidence, preserving leader-owned audit.`,
					taskIds: [taskOrId.id],
					taskReferences: [
						{
							id: taskOrId.id,
...
3. Request a claim via CLI interop (\`gjc team api claim-task --json\`)
4. Complete the work
5. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
6. Complete/fail via lifecycle transition API (\`gjc team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
7. Use \`gjc team api release-task-claim --json\` only for rollback to \`pending\`
...
export function generateTriggerMessage(
	workerName: string,
	teamName: string,
	teamStateRoot: string = ".gjc/state",
): string {
