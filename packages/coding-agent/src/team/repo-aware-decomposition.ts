...
import {
	...
	type TeamDagHandoff,
	type TeamDagNode,
	type TeamDagResolution,
	type TeamDagWorkerCountSource,
} from "./dag-schema.js";
...
function resolveWorkerCount(params: {
	...
	const requested = params.explicitWorkerCount ? params.requested : (policy?.requested_count ?? params.requested);
...
function buildFromDag(
	input: LegacyTeamExecutionPlanInput,
	resolution: TeamDagResolution & { dag: TeamDagHandoff },
): RepoAwareTeamExecutionPlan {
	const sorted = topologicalSort(resolution.dag.nodes);
	const readyLaneCount = firstReadyLaneCount(sorted);
	const useful = usefulLaneCount(
		sorted,
		readyLaneCount,
		resolution.dag.worker_policy?.reserve_verification_lane !== false,
	);
	...
	return {
		...
		metadata: {
			...
			...(input.approvedRepositoryContextSummary
				? { approved_context_summary: input.approvedRepositoryContextSummary }
				: {}),
		},
	};
}
...
export function remapRepoAwareDecompositionMetadataToCreatedTasks(
	...
	plannedTasks: Array<
		Pick<
			RepoAwareTask,
			"symbolic_id" | "symbolic_depends_on" | "lane" | "filePaths" | "domains" | "allocation_reason"
		>
	>,
...
export function buildRepoAwareTeamExecutionPlan(input: LegacyTeamExecutionPlanInput): RepoAwareTeamExecutionPlan {
	...
	const legacy = input.buildLegacyPlan(
		input.task,
		input.workerCount,
		input.agentType,
		input.explicitAgentType,
		input.explicitWorkerCount,
	);
	return {
		...legacy,
		metadata: {
