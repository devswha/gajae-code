import { type AllocationWorkerInput, chooseTaskOwner } from "./allocation-policy.js";
...
function isWorkerAvailable(worker: RebalanceWorkerInput): boolean {
	return (
		worker.alive &&
		(worker.status.state === "idle" || worker.status.state === "done" || worker.status.state === "unknown")
	);
}
