...
import { type DispatchRecord, getDefaultBridge, isBridgeEnabled, resolveBridgeStateDir } from "../runtime/bridge.js";
import {
...
export interface TeamConfig {
	...
	workers: WorkerInfo[];
	created_at: string;
	tmux_session: string; // "gjc-team-{name}"
	next_task_id: number;
	leader_cwd?: string;
...
export function normalizeTeamPolicy(
	...
	defaults: Pick<TeamPolicy, "display_mode" | "worker_launch_mode"> = {
		display_mode: "auto",
	...
	const dispatchMode =
		policy?.dispatch_mode === "transport_direct" ? "transport_direct" : "hook_preferred_with_fallback";
...
function resolveDisplayModeFromEnv(env: NodeJS.ProcessEnv): TeamPolicy["display_mode"] {
	const raw = readEnvValue(env, ["GJC_TEAM_DISPLAY_MODE", "GJC_TEAM_MODE"]);
	if (!raw) return "auto";
...
function resolveWorkerLaunchModeFromEnv(env: NodeJS.ProcessEnv): TeamPolicy["worker_launch_mode"] {
	const raw = readEnvValue(env, ["GJC_TEAM_WORKER_LAUNCH_MODE"]);
	if (!raw || raw === "interactive") return "interactive";
	if (raw === "prompt") return "prompt";
	throw new Error(`Invalid GJC_TEAM_WORKER_LAUNCH_MODE value "${raw}". Expected: interactive, prompt`);
}
...
function resolvePermissionsSnapshot(env: NodeJS.ProcessEnv): PermissionsSnapshot {
	...
	const sandboxMode = readEnvValue(env, ["GJC_SANDBOX_MODE", "CODEX_SANDBOX_MODE", "SANDBOX_MODE"]);
	if (sandboxMode) snapshot.sandbox_mode = sandboxMode;

	const network = parseOptionalBoolean(
		readEnvValue(env, ["GJC_NETWORK_ACCESS", "CODEX_NETWORK_ACCESS", "NETWORK_ACCESS"]),
	);
...
async function resolveLeaderSessionId(cwd: string, env: NodeJS.ProcessEnv): Promise<string> {
	const fromEnv = readEnvValue(env, ["GJC_SESSION_ID", "CODEX_SESSION_ID", "SESSION_ID"]);
...
// Team state directory: .gjc/state/team/{teamName}/
...
// Creates: .gjc/state/team/{name}/, workers/{worker-1}..{worker-N}/, tasks/
...
export async function initTeamState(
	...
	const leaderWorkerId = readEnvValue(env, ["GJC_TEAM_WORKER"]) ?? "leader-fixed";
	...
	const config: TeamConfig = {
		...
		tmux_session: `gjc-team-${teamName}`,
		next_task_id: 1,
...
export async function readTeamManifestV2(teamName: string, cwd: string): Promise<TeamManifestV2 | null> {
	try {
		...
		const legacyPolicy = parsedManifest.policy as
			| (Partial<TeamPolicy> &
		...
		return {
			...
			team_decomposition:
				parsedManifest.team_decomposition ??
				(legacyTeamDecomposition &&
				typeof legacyTeamDecomposition === "object" &&
				!Array.isArray(legacyTeamDecomposition)
...
export async function withScalingLock<T>(teamName: string, cwd: string, fn: () => Promise<T>): Promise<T> {
...
async function withTaskClaimLock<T>(
	...
	return await withTaskClaimLockImpl(
		teamName,
		taskId,
		cwd,
		LOCK_STALE_MS,
		{ teamDir, taskClaimLockDir, mailboxLockDir },
		fn,
	);
}

async function withMailboxLock<T>(teamName: string, workerName: string, cwd: string, fn: () => Promise<T>): Promise<T> {
	return await withMailboxLockImpl(
		teamName,
		workerName,
		cwd,
		LOCK_STALE_MS,
...
export async function updateTask(
	...
	const lock = await withTaskClaimLock(teamName, taskId, cwd, async () => {
		...
		if (
			...
			!["pending", "blocked", "in_progress", "completed", "failed"].includes(updates.status)
		) {
...
async function writeBridgeDispatchCompat(
	teamName: string,
...
export async function writeTaskApproval(teamName: string, approval: TaskApprovalRecord, cwd: string): Promise<void> {
...
function normalizeTeamLeaderAttentionState(teamName: string, raw: unknown): TeamLeaderAttentionState | null {
	if (!raw || typeof raw !== "object") return null;
	const parsed = raw as Record<string, unknown>;
	const source =
		parsed.source === "native_stop"
			? "native_stop"
			: parsed.source === "native_session_end"
				? "native_session_end"
				: "notify_hook";
	const leaderDecisionState =
		parsed.leader_decision_state === "done_waiting_on_leader" ||
	...
	const attentionReasons = Array.isArray(parsed.attention_reasons)
		? parsed.attention_reasons.filter(
...
export async function writeTeamPhase(teamName: string, phaseState: TeamPhaseState, cwd: string): Promise<void> {
	await writeTeamPhaseImpl(teamName, phaseState, cwd, teamPhasePath, writeAtomic);
}

export async function readTeamLeaderAttention(teamName: string, cwd: string): Promise<TeamLeaderAttentionState | null> {
...
export async function writeTeamLeaderAttention(
	...
	await writeAtomic(
		leaderAttentionPath(teamName, cwd),
		JSON.stringify(
			{
				...attentionState,
				team_name: teamName,
...
> {
	...
	return {
		...
		leader_attention_reason: leaderAttentionPending
			? (existing?.leader_attention_reason ?? "leader_session_stopped")
			: null,
		attention_reasons: [...attentionReasons],
...
export async function markTeamLeaderStopObserved(
	...
	const nextSource = existing?.source === "native_stop" && source === "native_session_end" ? "native_stop" : source;
...
export async function markOwnedTeamsLeaderStopObserved(
	...
	for (const entry of entries) {
		...
		const [manifest, phase] = await Promise.all([readTeamManifestV2(teamName, cwd), readTeamPhase(teamName, cwd)]);
