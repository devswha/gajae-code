...
const SIMPLE_SCOPE_PATTERNS = [/\bfix\b.*\btypo\b/i, /\bupdate\b.*\bcopy\b/i, /\brename\b.*\bsingle\b/i];
...
function isNarrowTask(text: string): boolean {
	return (
		NARROW_TASK_PATTERNS.some(pattern => pattern.test(text)) &&
		(SIMPLE_SCOPE_PATTERNS.some(pattern => pattern.test(text)) ||
			!/\b(search|debug|investigate|refactor|migration|review|test)\b/i.test(text))
	);
}
...
export function synthesizeDelegationPlan(
