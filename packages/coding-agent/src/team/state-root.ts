...
/**
 * Resolve the canonical GJC team state root for a leader working directory.
 */
export function resolveCanonicalTeamStateRoot(leaderCwd: string, env: NodeJS.ProcessEnv = process.env): string {
	const explicit = env.GJC_TEAM_STATE_ROOT;
	if (typeof explicit === "string" && explicit.trim() !== "") {
		return resolve(leaderCwd, explicit.trim());
	}

	const boxedRoot = env.GJC_ROOT || env.GJC_STATE_ROOT;
	if (typeof boxedRoot === "string" && boxedRoot.trim() !== "") {
		return resolve(leaderCwd, boxedRoot.trim(), ".gjc", "state");
	}

	return gjcStateDir(leaderCwd);
}
...
interface WorkerTeamStateRootResolveOptions {
	/**
	 * Allow probing cwd/.gjc/state as a last-resort candidate. This remains
...
async function cwdMatchesIdentityWorktree(
	...
	const [normalizedCwd, normalizedWorktree] = await Promise.all([normalizePath(cwd), normalizePath(worktreePath)]);
...
async function resolveWorkerTeamStateRootWithOptions(
	...
	const explicit = typeof env.GJC_TEAM_STATE_ROOT === "string" ? env.GJC_TEAM_STATE_ROOT.trim() : "";
	if (explicit) {
	...
	const leaderCwd = typeof env.GJC_TEAM_LEADER_CWD === "string" ? env.GJC_TEAM_LEADER_CWD.trim() : "";
	const leaderStateRoot = leaderCwd ? join(resolve(cwd, leaderCwd), ".gjc", "state") : "";
	const cwdStateRoot = join(cwd, ".gjc", "state");
	...
	const metadataSources: Array<["identity.json" | "manifest.v2.json" | "config.json", WorkerTeamStateRootSource]> = [
...
/**
 * Resolve the canonical team state root for an GJC team worker PostToolUse/git hook.
 *
 ...
 * local `.gjc/state` root and writing cross-worker runtime state in the wrong
 ...
 * must not invent `cwd/.gjc/state` when the runtime did not provide a canonical
