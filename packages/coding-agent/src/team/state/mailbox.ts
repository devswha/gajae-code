...
interface MailboxDeps {
	...
	readTeamConfig: (
		teamName: string,
		cwd: string,
	) => Promise<{ created_at?: string; workers: Array<{ name: string }> } | null>;
}
...
export async function sendDirectMessage(
	...
	await deps.withMailboxLock(deps.teamName, toWorker, deps.cwd, async () => {
		...
		const existing = dedupeCandidates.find(
			candidate =>
...
export async function markMessageDelivered(workerName: string, messageId: string, deps: MailboxDeps): Promise<boolean> {
...
export async function markMessageNotified(workerName: string, messageId: string, deps: MailboxDeps): Promise<boolean> {
...
export async function listMailboxMessages(workerName: string, deps: MailboxDeps): Promise<TeamMailboxMessage[]> {
	const mailbox = await deps.readMailbox(deps.teamName, workerName, deps.cwd);
