...
interface MonitorDeps {
	...
	readTeamConfig: (
		teamName: string,
		cwd: string,
	) => Promise<{ name: string; worker_count: number; workers: Array<{ name: string }> } | null>;
...
function normalizeWorkerIntegrationState(value: unknown): TeamWorkerIntegrationState | null {
	...
	return {
		last_seen_head:
			typeof raw.last_seen_head === "string" && raw.last_seen_head !== "" ? raw.last_seen_head : undefined,
		last_integrated_head:
			typeof raw.last_integrated_head === "string" && raw.last_integrated_head !== ""
				? raw.last_integrated_head
				: undefined,
		last_leader_head:
			typeof raw.last_leader_head === "string" && raw.last_leader_head !== "" ? raw.last_leader_head : undefined,
		last_rebased_leader_head:
			typeof raw.last_rebased_leader_head === "string" && raw.last_rebased_leader_head !== ""
				? raw.last_rebased_leader_head
				: undefined,
		status: isTeamWorkerIntegrationStatus(raw.status) ? raw.status : undefined,
		conflict_commit:
			typeof raw.conflict_commit === "string" && raw.conflict_commit !== "" ? raw.conflict_commit : undefined,
...
): Promise<TeamSummarySnapshot | null> {
