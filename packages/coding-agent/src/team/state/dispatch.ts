...
export function normalizeDispatchRequest(
	...
	return sanitizeDispatchRequestForStatus({
		...
		inbox_correlation_key:
			typeof raw.inbox_correlation_key === "string" && raw.inbox_correlation_key !== ""
...
export function normalizeBridgeDispatchRecord(
	...
	const metadataTeamName =
		typeof metadata.team_name === "string" && metadata.team_name.trim() !== "" ? metadata.team_name.trim() : null;
	...
	return normalizeDispatchRequest(
		teamName,
		{
			...
			message_id:
				typeof metadata.message_id === "string" && metadata.message_id !== "" ? metadata.message_id : undefined,
			inbox_correlation_key:
			...
			fallback_allowed: typeof metadata.fallback_allowed === "boolean" ? metadata.fallback_allowed : undefined,
...
export async function markDispatchRequestFailed(requestId: string, reason: string, deps: DispatchDeps): Promise<void> {
