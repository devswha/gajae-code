...
import { gjcStateDir } from "../utils/paths.js";
import {
...
async function markLatestMailboxDispatchDelivered(
	...
	const delivered = await teamMarkDispatchRequestDelivered(
		teamName,
		latest.request_id,
...
function readLatestLeaderRuntimeActivityMs(cwd: string): number {
	const path = join(gjcStateDir(cwd), "leader-runtime-activity.json");
...
function buildStallState(
	...
	const workerNames = listTeamWorkerNames(summary, snapshot);
	const deadWorkers = workerNames.filter(
	...
	const terminalLeaderDecisionPending =
		pendingTaskCount === 0 && idleState.all_workers_idle === true && liveWorkers.length > 0;
	...
	const authoritativeDecisionState = teamTerminal ? "still_actionable" : leaderDecisionState;
	...
	const leaderAttentionPending =
		!teamTerminal && (unreadLeaderMessageCount > 0 || pendingLeaderDispatchCount > 0 || leaderSessionStopped);
	const leaderStale = !teamTerminal && leaderSessionStopped;
	const teamStalled =
		stalledWorkers.length > 0 || leaderAttentionPending || (deadWorkers.length > 0 && pendingTaskCount > 0);
...
function resolveTeamWorkingDirectory(teamName: string, preferredCwd: string): string {
	...
	const envTeamStateRoot = process.env.GJC_TEAM_STATE_ROOT;
...
function readLegacyMailboxMessages(teamName: string, workerName: string, cwd: string): Array<Record<string, unknown>> {
...
function readLegacyMailboxMessage(
	...
	return (
		readLegacyMailboxMessages(teamName, workerName, cwd).find(
			message => String(message.message_id || "") === messageId,
		) ?? null
	);
}
...
export function buildLegacyTeamDeprecationHint(legacyName: string, originalArgs?: Record<string, unknown>): string {
	...
	if (!operation) {
		return `Use CLI interop: gjc team api <operation> --input '${payload}' --json`;
	}
	return `Use CLI interop: gjc team api ${operation} --input '${payload}' --json`;
}
...
function assertUnsafeTeamNameMatchesKnownDisplay(rawTeamName: string, cwd: string): void {
	...
	const matchesKnownDisplay = listTeamLookupCandidates(cwd).some(candidate => {
		return (
			normalizeTeamDisplayLookupName(candidate.displayName) === normalized ||
...
export async function executeTeamApiOperation(
	...
	try {
		...
		const resolvedTeamName = rawTeamNameForCwd
			? resolveTeamNameForCurrentContext(rawTeamNameForCwd, fallbackCwd)
			: "";
		...
		switch (operation) {
			case "send-message": {
				...
				if (!fromWorker) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "from_worker is required. You must identify yourself." },
					};
				}
				if (!teamName || !toWorker || !body) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, from_worker, to_worker, body are required" },
					};
				}
				...
				if (!config) {
					return {
						ok: false,
						operation,
						error: { code: "team_not_found", message: `Team ${teamName} not found` },
					};
				}

				const recipient =
					toWorker === "leader-fixed" ? null : (config.workers.find(worker => worker.name === toWorker) ?? null);
				if (toWorker !== "leader-fixed" && !recipient) {
					return {
						ok: false,
						operation,
						error: { code: "worker_not_found", message: `Worker ${toWorker} not found in team ${teamName}` },
					};
				}
				...
				const livePaneId = toWorker === "leader-fixed" ? (config.leader_pane_id ?? undefined) : recipient?.pane_id;
				...
				const legacyMatching =
					!matching && outcome.message_id
						? readLegacyMailboxMessage(teamName, toWorker, outcome.message_id, cwd)
						: null;
				const legacySenderFallback =
					!matching && !legacyMatching
						? ([...readLegacyMailboxMessages(teamName, toWorker, cwd)]
								.reverse()
								.find(
									message =>
				...
				if (!persisted) {
					throw new Error(
						`send-message could not locate persisted mailbox message for ${fromWorker} -> ${toWorker}`,
					);
				}
				const message = persisted.body || !body ? persisted : { ...persisted, body };
			...
			case "broadcast": {
				...
				if (!teamName || !fromWorker || !body) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, from_worker, body are required" },
					};
				}
			...
			case "mailbox-list": {
				...
				if (!teamName || !worker) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name and worker are required" },
					};
				}
			...
			case "mailbox-mark-delivered": {
				...
				if (!teamName || !worker || !messageId) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, worker, message_id are required" },
					};
				}
			...
			case "create-task": {
				...
				if (!teamName || !subject || !description) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, subject, description are required" },
					};
				}
			...
			case "read-task": {
				...
				if (!teamName || !taskId) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name and task_id are required" },
					};
				}
				...
				if ("subject" in args) {
					if (typeof opArgs.subject !== "string") {
						return {
							ok: false,
							operation,
							error: { code: "invalid_input", message: "subject must be a string when provided" },
						};
					}
				...
				if ("description" in args) {
					if (typeof opArgs.description !== "string") {
						return {
							ok: false,
							operation,
							error: { code: "invalid_input", message: "description must be a string when provided" },
						};
					}
				...
				if ("requires_code_change" in args) {
					if (typeof opArgs.requires_code_change !== "boolean") {
						return {
							ok: false,
							operation,
							error: { code: "invalid_input", message: "requires_code_change must be a boolean when provided" },
						};
					}
			...
			case "claim-task": {
				...
				if (!teamName || !taskId || !worker) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, task_id, worker are required" },
					};
				}
			...
			case "transition-task-status": {
				...
				if (!teamName || !taskId || !from || !to || !claimToken) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, task_id, from, to, claim_token are required" },
					};
				}
				...
				if (!allowed.has(from) || !allowed.has(to)) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "from and to must be valid task statuses" },
					};
				}
				if (transitionResult !== undefined && typeof transitionResult !== "string") {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "result must be a string when provided" },
					};
				}
				if (transitionError !== undefined && typeof transitionError !== "string") {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "error must be a string when provided" },
					};
				}
			...
			case "release-task-claim": {
				...
				if (!teamName || !taskId || !claimToken || !worker) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, task_id, claim_token, worker are required" },
					};
				}
			...
			case "update-worker-heartbeat": {
				...
				if (
					...
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, worker, pid, turn_count, alive are required" },
					};
				}
				await teamUpdateWorkerHeartbeat(
					teamName,
					worker,
					{ pid, turn_count: turnCount, alive, last_turn_at: new Date().toISOString() },
					cwd,
				);
			...
			case "write-worker-inbox": {
				...
				if (!teamName || !worker || !content) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, worker, content are required" },
					};
				}
			...
			case "write-worker-identity": {
				...
				if (!teamName || !worker || typeof index !== "number" || !role) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, worker, index, role are required" },
					};
				}
			...
			case "append-event": {
				...
				if (!teamName || !eventType || !worker) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, type, worker are required" },
					};
				}
				if (!TEAM_EVENT_TYPES.includes(eventType as TeamEventType)) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: `type must be one of: ${TEAM_EVENT_TYPES.join(", ")}` },
					};
				}
			...
			case "read-events": {
				...
				const events = await readTeamEvents(teamName, cwd, {
					afterEventId:
						typeof opArgs.after_event_id === "string" ? opArgs.after_event_id.trim() || undefined : undefined,
				...
				return {
					...
					data: {
						...
						cursor:
							...
							(typeof opArgs.after_event_id === "string" ? opArgs.after_event_id.trim() : ""),
						events,
					},
				};
			}
			...
			case "read-stall-state": {
				...
				const [
					...
					phaseState,
					leaderAttention,
					leaderMailboxMessages,
					leaderDispatchRequests,
					latestLeaderActivityMs,
				] = await Promise.all([
					teamGetSummary(teamName, cwd),
					teamReadMonitorSnapshot(teamName, cwd),
					readTeamEvents(teamName, cwd),
				...
				const pendingLeaderDispatchCount = leaderDispatchRequests.filter(
					(request: { status?: string }) => request.status === "pending" || request.status === "notified",
				).length;
			...
			case "write-shutdown-request": {
				...
				if (!teamName || !worker || !requestedBy) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name, worker, requested_by are required" },
					};
				}
			...
			case "write-monitor-snapshot": {
				...
				if (!teamName || !snapshot) {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "team_name and snapshot are required" },
					};
				}
			...
			case "write-task-approval": {
				...
				if (rawRequired !== undefined && typeof rawRequired !== "boolean") {
					return {
						ok: false,
						operation,
						error: { code: "invalid_input", message: "required must be a boolean when provided" },
					};
				}
		...
		if (error instanceof TeamLookupAmbiguityError) {
			return {
				ok: false,
				operation,
				error: { code: "ambiguous_team_name", message: error.message, details: { candidates: error.candidates } },
			};
		}
		return {
			ok: false,
