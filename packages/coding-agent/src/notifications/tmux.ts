...
const GJC_METADATA_SEGMENT_RE = /^\[GJC(?:[#\]].*)?$/;
const HUD_STATUS_SEGMENT_RE =
	/^(?:ralph:\d+\/(?:\d+|\?)|autopilot:[\w-]+|ralplan:(?:\d+\/(?:\d+|\?)|[\w-]+)|interview:[\w:-]+|research:[\w-]+|qa:[\w-]+|team:(?:\d+\s+workers|[\w.-]+)|ultrawork|turns:\d+|tokens:[\dkm.]+|quota:[\w%,.]+|session:[\dhms]+|last:\d+[smh](?:\s+ago)?|total-turns:\d+|tmux:[\w:.-]+)$/i;
const BRANCH_METADATA_SEGMENT_RE =
	/^(?:(?:fix|feat|feature|chore|refactor|hotfix|release|docs|doc|test|tests|ci|build|perf|revert|bugfix|spike|wip)\/[A-Za-z0-9._/-]+|HEAD(?: -> [A-Za-z0-9._/-]+)?|detached)$/;

function isMetadataOnlyTmuxSegment(segment: string): boolean {
	return (
		GJC_METADATA_SEGMENT_RE.test(segment) ||
...
function isMetadataOnlyTmuxLine(line: string): boolean {
	...
	const hasExplicitStatusSegment = segments.some(
		segment => GJC_METADATA_SEGMENT_RE.test(segment) || HUD_STATUS_SEGMENT_RE.test(segment),
	);
...
function shouldUsePidFallback(): boolean {
	return process.env.GJC_TMUX_PID_FALLBACK === "1";
}
	...
	if (process.env.TMUX) {
		try {
			...
			const sessionName = execTmux(
				paneTargetSafe ? ["display-message", "-p", "-t", paneTargetSafe, "#S"] : ["display-message", "-p", "#S"],
			);
	...
	try {
		...
		for (const line of output.split("\n")) {
			...
			if (parts.length === 2) {
				...
				if (!Number.isNaN(pid)) panePids.set(pid, parts[1]);
			}
		}
...
export function captureTmuxPaneWithLiveness(paneId?: string | null, lines: number = 12): TmuxPaneCaptureResult {
	...
	try {
		...
		const output = execFileSync(
			...
			buildCapturePaneArgv(target, clampedLines),
			{
				encoding: "utf-8",
				timeout: 3000,
				stdio: ["pipe", "pipe", "pipe"],
