...
export function analyzePaneContent(content: string): PaneAnalysis {
	...
	const hasCodex =
		lower.includes("codex") || lower.includes("gjc") || lower.includes("gajae-code") || lower.includes("openai");

	const hasRateLimitMessage = lower.includes("rate limit") || lower.includes("rate-limit") || lower.includes("429");

	const isBlocked = lower.includes("waiting") || lower.includes("blocked") || lower.includes("paused");
 ...
 */
export function buildSendPaneArgvs(paneId: string, text: string, pressEnter: boolean = true): string[][] {
...
export function getSubmitPresses(workerCli: string): number {
	if (process.env.GJC_RUNTIME_BRIDGE === "0") {
