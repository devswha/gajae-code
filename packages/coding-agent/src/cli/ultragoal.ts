...
import {
	...
	GJCGoalSnapshotError,
	readGJCGoalSnapshotInput,
	reconcileGJCGoalSnapshot,
} from "../goal-workflows/gjc-goal-snapshot.js";
import {
	addUltragoalGoal,
	buildGJCGoalInstruction,
	checkpointUltragoal,
	createUltragoalPlan,
	readUltragoalPlan,
...
export const ULTRAGOAL_HELP = `gjc ultragoal - Durable repo-native multi-goal workflow over GJC goal mode

Usage:
  gjc ultragoal create-goals [--brief <text> | --brief-file <path> | --from-stdin] [--goal <title::objective>] [--gjc-goal-mode <aggregate|per-story>] [--force] [--json]
  gjc ultragoal complete-goals [--retry-failed] [--json]
  gjc ultragoal add-goal --title <title> --objective <text> [--evidence <text>] [--json]
  gjc ultragoal steer --kind <mutation-kind> --evidence <text> --rationale <text> [--target-goal-id <id> | --target-goal-ids <id1,id2,...>] [--title <title>] [--objective <text>] [--json]
  gjc ultragoal record-review-blockers --goal-id <id> --title <title> --objective <text> --evidence <review-findings> --gjc-goal-json <active-json-or-path> [--json]
  gjc ultragoal steer --kind <add_subgoal|split_subgoal|reorder_pending|revise_pending_wording|annotate_ledger|mark_blocked_superseded> --evidence <text> --rationale <text> [--target-goal-id <id>] [--title <text>] [--objective <text>] [--after-json <json-or-path>] [--idempotency-key <key>] [--json]
  gjc ultragoal steer --directive-json <json-or-path> [--json]
  gjc ultragoal checkpoint --goal-id <id> --status <complete|failed|blocked> [--evidence <text>] [--gjc-goal-json <json-or-path>] [--quality-gate-json <json-or-path>] [--json]
  gjc ultragoal status [--gjc-goal-json <json-or-path>] [--json]
...
GJC goal integration:
  ...
  the active GJC agent when to call get_goal/create_goal/update_goal safely.
  Ultragoal does not call /goal clear or hidden thread/goal/clear routes. For
  multiple sequential ultragoal runs in one GJC session/thread, manually run
  /goal clear in the GJC UI before creating the next aggregate goal.
  New plans default to aggregate mode: one GJC goal covers the whole ultragoal
  run while GJC checkpoints G001/G002 stories in the durable ledger. Legacy
  ...
  audits accepted/rejected/deduped results in .gjc/ultragoal/ledger.jsonl, and
...
function normalizeGJCGoalMode(raw: string | undefined): "aggregate" | "per_story" | undefined {
	if (!raw) return undefined;
	if (raw === "aggregate") return "aggregate";
	if (raw === "per-story" || raw === "per_story") return "per_story";
	throw new UltragoalError("Invalid --gjc-goal-mode; expected aggregate or per-story.");
}
...
function printStatus(plan: Awaited<ReturnType<typeof readUltragoalPlan>>): void {
	...
	if (summary.aggregateComplete) {
		...
		console.log(
			`microgoal ledger bookkeeping (progress-only): ${summary.complete}/${summary.total} complete, ${summary.pending} pending, ${summary.inProgress} in progress, ${summary.failed} failed, ${summary.reviewBlocked} review-blocked, ${summary.needsUserDecision} needs-user-decision`,
		);
	} else if (summary.artifactComplete) {
		console.log("ultragoal artifact goals: complete");
		console.log(
			`gjc goal reconciliation: not recorded in GJC aggregateCompletion; status is artifact-backed until a fresh GJC goal snapshot is available.`,
		);
		console.log(
			`microgoal ledger: ${summary.complete}/${summary.total} complete, ${summary.pending} pending, ${summary.inProgress} in progress, ${summary.failed} failed, ${summary.reviewBlocked} review-blocked, ${summary.needsUserDecision} needs-user-decision`,
		);
	} else {
		console.log(
			`ultragoal: ${summary.complete}/${summary.total} complete, ${summary.pending} pending, ${summary.inProgress} in progress, ${summary.failed} failed, ${summary.reviewBlocked} review-blocked, ${summary.needsUserDecision} needs-user-decision`,
		);
	}
...
async function parseGJCGoalJson(raw: string | undefined): Promise<unknown> {
	if (!raw) return undefined;
	return readGJCGoalSnapshotInput(raw, process.cwd());
}
...
function parseSteeringKind(raw: string | undefined): UltragoalSteeringMutationKind {
	...
	if (!STEERING_KINDS.has(raw as UltragoalSteeringMutationKind))
		throw new UltragoalError(`Invalid --kind: ${raw}. Expected one of ${Array.from(STEERING_KINDS).join(", ")}.`);
	return raw as UltragoalSteeringMutationKind;
}

function parseSteeringSource(
	raw: string | undefined,
	fallback: UltragoalSteeringSource = "cli",
): UltragoalSteeringSource {
	if (!raw) return fallback;
	if (!STEERING_SOURCES.has(raw as UltragoalSteeringSource))
		throw new UltragoalError(`Invalid --source: ${raw}. Expected one of ${Array.from(STEERING_SOURCES).join(", ")}.`);
	return raw as UltragoalSteeringSource;
}

function assertPlainObject(value: unknown, label: string): Record<string, unknown> {
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw new UltragoalError(`${label} must be a JSON object.`);
...
function normalizeTargetGoalId(raw: Record<string, unknown>): string | undefined {
	...
	if (Array.isArray(raw.targetGoalIds))
		return raw.targetGoalIds.find((id): id is string => typeof id === "string" && id.trim().length > 0)?.trim();
	return undefined;
}

function normalizeSteeringProposal(
	raw: Record<string, unknown>,
	_fallbackDirectiveText?: string,
	...
	return {
		...
		after,
		pendingOrder: Array.isArray(raw.pendingOrder)
			? raw.pendingOrder.filter((id): id is string => typeof id === "string")
			: undefined,
...
async function parseSteeringProposal(args: readonly string[]): Promise<UltragoalSteeringProposal> {
	...
	const directiveRaw =
		readValue(args, "--directive-json") ?? (directiveFile ? await readFile(directiveFile, "utf-8") : undefined);
	if (directiveRaw) {
	...
	if (freeform)
		throw new UltragoalError(
			"gjc ultragoal steer rejects broad natural-language mutation requests; pass structured fields or --directive-json.",
		);
	...
	try {
		...
		if (command === "create" || command === "create-goals") {
			...
			const plan = await createUltragoalPlan(cwd, {
				brief,
				goals,
				gjcGoalMode: normalizeGJCGoalMode(readValue(rest, "--gjc-goal-mode")),
		...
		if (command === "status") {
			...
			const snapshot = await readGJCGoalSnapshotInput(readValue(rest, "--gjc-goal-json"), cwd);
			const activeGoal = plan.goals.find(goal => goal.id === plan.activeGoalId || goal.status === "in_progress");
			const expectedObjective = plan.gjcGoalMode === "aggregate" ? plan.gjcObjective : activeGoal?.objective;
			const reconciliation =
				activeGoal || snapshot
					? reconcileGJCGoalSnapshot(snapshot, {
							expectedObjective: expectedObjective ?? plan.gjcObjective ?? "",
							acceptedObjectives: plan.gjcGoalMode === "aggregate" ? plan.gjcObjectiveAliases : undefined,
							allowedStatuses:
								activeGoal && plan.gjcGoalMode === "aggregate" ? ["active"] : ["active", "complete"],
							requireSnapshot: false,
						})
					: null;
			const gjcGoalFallback =
				reconciliation?.snapshot.unavailableReason === "db_schema_context_error"
					? {
							status: "gjc_goal_reconciliation_unavailable",
							reason: reconciliation.snapshot.unavailableReason,
							message:
								"GJC goal DB/schema/context is unavailable; artifact-backed Ultragoal status remains available, but strict GJC goal completion reconciliation is deferred.",
						}
					: undefined;
			if (json) printJson({ plan, summary: summarizeUltragoalPlan(plan), reconciliation, gjcGoalFallback });
			else {
				printStatus(plan);
				if (gjcGoalFallback) console.log(`gjc goal fallback: ${gjcGoalFallback.message}`);
				if (reconciliation && !reconciliation.ok)
					console.log(`gjc goal warning: ${formatGJCGoalReconciliation(reconciliation)}`);
		...
		if (command === "record-review-blockers") {
			...
			const gjcGoal = await parseGJCGoalJson(readValue(rest, "--gjc-goal-json"));
			const result = await recordFinalReviewBlockers(cwd, { goalId, title, objective, evidence, gjcGoal });
			if (json)
				printJson({
					ok: true,
					plan: result.plan,
		...
		if (command === "complete" || command === "complete-goals" || command === "next" || command === "start-next") {
			...
			const instruction = buildGJCGoalInstruction(result.goal, result.plan);
		...
		if (command === "checkpoint") {
			...
			const plan = await checkpointUltragoal(cwd, { goalId, status, evidence, gjcGoal, qualityGate });
			if (json) printJson({ ok: true, plan, summary: summarizeUltragoalPlan(plan) });
			else {
		...
		if (error instanceof UltragoalError || error instanceof GJCGoalSnapshotError) {
