...
import { readPersistedSetupPreferencesSync, readPersistedSetupScopeSync } from "./setup-preferences.js";
...
export function resolveCodexConfigPathForLaunch(cwd: string, env: NodeJS.ProcessEnv = process.env): string {
	const codexHomeOverride = resolveCodexHomeForLaunch(cwd, env);
	return codexHomeOverride ? join(codexHomeOverride, "config.toml") : codexConfigPath();
}
