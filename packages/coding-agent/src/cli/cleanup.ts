...
import { resolveGjcFirstPartyMcpEntrypointForPluginTarget } from "../config/gjc-first-party-mcp.js";

const HELP = [
	"Usage: gjc cleanup [--dry-run]",
	"",
	"Kill orphaned GJC MCP server processes and remove stale GJC /tmp directories left behind by previous Codex App sessions.",
	"",
	"Options:",
...
const GJC_MCP_ENTRYPOINT_PATTERN =
	/(?:^|[\\/])dist[\\/]mcp[\\/]((?:state|memory|code-intel|trace|wiki)-server\.(?:[cm]?js|ts))\b/i;
const GJC_MCP_SERVE_TARGET_PATTERN = /(?:^|\s)mcp-serve\s+([^\s]+)/i;
const CODEX_PROCESS_PATTERN = /(?:^|[\\/\s])codex(?:\.js)?(?:\s|$)|@openai[\\/]codex/i;
const GJC_LAUNCH_PROCESS_PATTERN =
	/(?:^|[\\/\s])gjc(?:\.js)?(?:\s|$)|(?:^|[\\/])(?:bin|dist[\\/]cli)[\\/]gjc\.js(?:\s|$)|gajae-code[\\/]dist[\\/]cli[\\/]gjc\.js/i;
const GJC_TMP_DIRECTORY_PATTERN = /^(omc|gjc|gajae-code)-/;
...
export function isGjcMcpProcess(command: string): boolean {
	return extractGjcMcpEntrypoint(command) !== null;
}

export function extractGjcMcpEntrypoint(command: string): string | null {
	const normalized = normalizeCommand(command);
	const directEntrypoint = normalized.match(GJC_MCP_ENTRYPOINT_PATTERN)?.[1]?.toLowerCase();
	if (directEntrypoint) return directEntrypoint;

	const mcpServeTarget = normalized.match(GJC_MCP_SERVE_TARGET_PATTERN)?.[1];
	return resolveGjcFirstPartyMcpEntrypointForPluginTarget(mcpServeTarget);
}
...
function listWindowsGjcProcesses(runCommand: ProcessListCommandRunner): ProcessEntry[] {
...
export function listGjcProcesses(
	runCommand: ProcessListCommandRunner = defaultProcessListCommandRunner,
): ProcessEntry[] {
	if (process.platform === "win32") return listWindowsGjcProcesses(runCommand);
...
function isGjcLaunchProcess(command: string): boolean {
	return GJC_LAUNCH_PROCESS_PATTERN.test(normalizeCommand(command));
}
...
function resolveProtectedRootPid(processes: readonly ProcessEntry[], currentPid: number): number {
...
export function buildProtectedPidSet(processes: readonly ProcessEntry[], currentPid: number): Set<number> {
...
export function findDuplicateSiblingCleanupCandidates(processes: readonly ProcessEntry[]): CleanupCandidate[] {
	const groups = new Map<string, ProcessEntry[]>();

	for (const processEntry of processes) {
		const entrypoint = extractGjcMcpEntrypoint(processEntry.command);
...
export function findCleanupCandidates(processes: readonly ProcessEntry[], currentPid: number): CleanupCandidate[] {
	const protectedPids = buildProtectedPidSet(processes, currentPid);
	const duplicateCandidates = findDuplicateSiblingCleanupCandidates(processes).filter(
		processEntry => processEntry.pid !== currentPid,
	);
	...
	const orphanCandidates = processes
		...
		.filter(processEntry => isGjcMcpProcess(processEntry.command))
	...
	return [...duplicateCandidates, ...orphanCandidates].sort((left, right) => left.pid - right.pid);
}
...
export function findLaunchSafeCleanupCandidates(
	...
	const processByPid = new Map(processes.map(processEntry => [processEntry.pid, processEntry] as const));

	return findCleanupCandidates(processes, currentPid).filter(candidate => {
		if (candidate.ppid <= 1) return true;

		// Launch-safe cleanup runs automatically before starting Codex/GJC work.
		// Preserve every MCP process still attached to a live Codex or GJC launch
		...
		// findCleanupCandidates/default cleanupGjcMcpProcesses selection.
		return (
			!hasAncestorMatching(processByPid, candidate.pid, isCodexSessionProcess) &&
			!hasAncestorMatching(processByPid, candidate.pid, isGjcLaunchProcess)
		);
	});
}
...
export async function cleanupGjcMcpProcesses(
	args: readonly string[],
	...
	const listProcessesImpl = dependencies.listProcesses ?? listGjcProcesses;
	...
	if (candidates.length === 0) {
		writeLine(
			dryRun
				? "Dry run: no orphaned GJC MCP server processes found."
				: "No orphaned GJC MCP server processes found.",
		);
		return {
			dryRun,
			candidates,
	...
	if (dryRun) {
		writeLine(`Dry run: would terminate ${candidates.length} orphaned GJC MCP server process(es):`);
	...
	writeLine(`Found ${candidates.length} orphaned GJC MCP server process(es). Sending SIGTERM...`);
	...
	if (stillRunning.length > 0) {
		writeLine(
			`Escalating to SIGKILL for ${stillRunning.length} process(es) still alive after ${SIGTERM_GRACE_MS / 1000}s.`,
		);
	...
	writeLine(
		`Killed ${terminatedCount} orphaned GJC MCP server process(es)${forceKilledCount > 0 ? ` (${forceKilledCount} required SIGKILL)` : ""}.`,
	);
...
export async function cleanupStaleTmpDirectories(
	...
	for (const entry of await listTmpEntries(tmpRoot)) {
		if (!entry.isDirectory() || !GJC_TMP_DIRECTORY_PATTERN.test(entry.name)) continue;
	...
	if (staleDirectories.length === 0) {
		writeLine(dryRun ? "Dry run: no stale GJC /tmp directories found." : "No stale GJC /tmp directories found.");
		return 0;
	}

	const summaryTarget = formatPlural(
		staleDirectories.length,
		"stale GJC /tmp directory",
		"stale GJC /tmp directories",
	);
	if (dryRun) {
	...
	writeLine(`Removed ${formatPlural(removedCount, "stale GJC /tmp directory", "stale GJC /tmp directories")}.`);
	return removedCount;
}

export async function cleanupCommand(args: string[], dependencies: CleanupCommandDependencies = {}): Promise<void> {
	const cleanupProcesses = dependencies.cleanupProcesses ?? cleanupGjcMcpProcesses;
