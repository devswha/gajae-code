...
function persistExactTeamModeState(cwd: string, updates: Record<string, unknown>, sessionId?: string): boolean {
...
function resolveApprovedTeamFollowupContext(cwd: string, task: string): TeamFollowupContext | null {
	...
	const persistedTeamName = typeof existingTeamState?.team_name === "string" ? existingTeamState.team_name.trim() : "";
	const persistedTeamStateRoot =
		typeof existingTeamState?.team_state_root === "string" && existingTeamState.team_state_root.trim() !== ""
	...
	const persistedWorkerCount =
		typeof existingTeamState?.agent_count === "number"
			...
			: typeof existingTeamState?.workerCount === "number"
				...
				: null;
	const persistedAgentType =
		typeof existingTeamState?.agentType === "string" && existingTeamState.agentType.trim() !== ""
			? existingTeamState.agentType.trim()
			: undefined;
	const persistedLinkedRalph =
		typeof existingTeamState?.linkedRalph === "boolean" ? existingTeamState.linkedRalph : undefined;
...
function buildExplicitGjcTeamLaunchCommand(
	task: string,
	workerCount: number,
	explicitWorkerCount: boolean,
	agentType: string,
	...
	const countToken = explicitAgentType ? `${workerCount}:${agentType}` : String(workerCount);
	return `gjc team ${countToken} ${JSON.stringify(task)}`;
}
...
const TEAM_HELP = `
Usage: gjc team [N:agent-type] "<task description>"
       gjc team status <team-name> [--json] [--tail-lines <100-1000>] [--model-inspect]
       gjc team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]
       gjc team resume <team-name>
       gjc team shutdown <team-name> [--force] [--confirm-issues]
       gjc team api <operation> [--input <json>] [--json]
       gjc team api --help

Notes:
  team workers use dedicated worktrees automatically by default.
  --worktree is deprecated for gjc team and is now only a backward-compatible no-op override.
  gjc team is a tmux-runtime surface by default; launch GJC CLI from a shell before starting teammates.
  use GJC-native subagents for small in-session fanout; use gjc team for durable tmux/state/worktree coordination.
  repo-aware DAG handoff is opt-in: Team only imports a DAG when the invocation matches the latest approved PRD/test-spec launch hint (or a short approved follow-up like \`gjc team team\`).

Examples:
  gjc team 3:executor "fix failing tests"
  gjc team status my-team
  gjc team status my-team --json
  gjc team status my-team --model-inspect --tail-lines 600
  gjc team api send-message --input '{"team_name":"my-team","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK"}' --json
`;

const TEAM_API_HELP = `
Usage: gjc team api <operation> [--input <json>] [--json]
       gjc team api <operation> --help

Supported operations:
  ${TEAM_API_OPERATIONS.join("\n  ")}

Examples:
  gjc team api list-tasks --input '{"team_name":"my-team"}' --json
  gjc team api claim-task --input '{"team_name":"my-team","task_id":"1","worker":"worker-1","expected_version":1}' --json

Safety:
  team status prints raw tmux capture commands by default so inspect hints do not spend Spark/model quota.
  pass --model-inspect to print gjc sparkshell summary commands intentionally.
`;
...
const TEAM_API_OPERATION_NOTES: Partial<Record<TeamApiOperation, string>> = {
	...
	cleanup:
		"Uses the runtime shutdown contract; add confirm_issues=true when failed tasks are acknowledged and shutdown should still proceed.",
	"orphan-cleanup": "Destructive escape hatch for known orphan recovery. Bypasses shutdown orchestration.",
	"read-events":
		"Events are returned in canonical form; worker_idle log entries normalize to type worker_state_changed with source_type worker_idle. wakeable_only defaults to false; set wakeable_only=true to mirror gjc team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).",
	"await-event":
		"Waits for the next matching event and returns status=timeout when no matching event arrives before timeout_ms. wakeable_only defaults to false; set wakeable_only=true to mirror gjc team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).",
	"read-idle-state":
	...
	switch (field) {
		...
		case "metadata":
			return {
				...
				diff_path: "/tmp/team/worktrees/worker-1/.gjc/diff.md",
...
function buildTeamApiOperationHelp(operation: TeamApiOperation): string {
	...
	const required = requiredFields.length > 0 ? requiredFields.map(field => `  - ${field}`).join("\n") : "  (none)";
	const optional =
...
${required}${optional}${note}Example:
  gjc team api ${operation} --input '${sampleInputJson}' --json
`.trim();
}
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (token === "--tail-lines") {
			...
			if (!Number.isFinite(parsed) || parsed < MIN_SPARKSHELL_TAIL_LINES || parsed > MAX_SPARKSHELL_TAIL_LINES) {
				throw new Error(
					`Usage: gjc team status <team-name> [--json] [--tail-lines <${MIN_SPARKSHELL_TAIL_LINES}-${MAX_SPARKSHELL_TAIL_LINES}>]`,
				);
			}
			return parsed;
		}
...
function parseTeamApiArgs(args: string[]): {
	...
	for (let i = 1; i < args.length; i += 1) {
		...
		throw new Error(`Unknown argument for "gjc team api": ${token}`);
	}
...
function inspectEntryDescriptors(paneStatus: TeamPaneStatus): InspectEntryDescriptor<unknown>[] {
	...
	return [
		...
		{
			prefix: "inspect_turns_without_progress",
			entries: paneStatus.recommended_inspect_turns_without_progress,
			shouldLog: isNumber,
		},
		{
			prefix: "inspect_last_turn_at",
			entries: paneStatus.recommended_inspect_last_turn_at,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_status_updated_at",
			entries: paneStatus.recommended_inspect_status_updated_at,
			shouldLog: isPresentString,
		},
		{ prefix: "inspect_pid", entries: paneStatus.recommended_inspect_pids, shouldLog: isNumber },
		{
			prefix: "inspect_worktree_path",
			entries: paneStatus.recommended_inspect_worktree_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worktree_repo_root",
			entries: paneStatus.recommended_inspect_worktree_repo_roots,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worktree_branch",
			entries: paneStatus.recommended_inspect_worktree_branches,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worktree_detached",
			entries: paneStatus.recommended_inspect_worktree_detached,
			shouldLog: isBoolean,
		},
		{
			prefix: "inspect_worktree_created",
			entries: paneStatus.recommended_inspect_worktree_created,
			shouldLog: isBoolean,
		},
		{
			prefix: "inspect_team_state_root",
			entries: paneStatus.recommended_inspect_team_state_roots,
			shouldLog: isPresentString,
		},
		{ prefix: "inspect_workdir", entries: paneStatus.recommended_inspect_workdirs, shouldLog: isPresentString },
		{
			prefix: "inspect_assigned_tasks",
			entries: paneStatus.recommended_inspect_assigned_tasks,
			shouldLog: hasItems,
			formatValue: joinSpace,
		},
		{
			prefix: "inspect_task_status",
			entries: paneStatus.recommended_inspect_task_statuses,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_result",
			entries: paneStatus.recommended_inspect_task_results,
			shouldLog: isPresentString,
		},
		...
		{
			prefix: "inspect_task_created_at",
			entries: paneStatus.recommended_inspect_task_created_at,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_completed_at",
			entries: paneStatus.recommended_inspect_task_completed_at,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_depends_on",
			entries: paneStatus.recommended_inspect_task_depends_on,
			shouldLog: hasItems,
			formatValue: joinSpace,
		},
		{
			prefix: "inspect_task_claim_present",
			entries: paneStatus.recommended_inspect_task_claim_present,
			shouldLog: isBoolean,
		},
		{
			prefix: "inspect_task_claim_owner",
			entries: paneStatus.recommended_inspect_task_claim_owners,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_claim_token",
			entries: paneStatus.recommended_inspect_task_claim_tokens,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_claim_leased_until",
			entries: paneStatus.recommended_inspect_task_claim_leases,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_task_claim_lock_path",
			entries: paneStatus.recommended_inspect_task_claim_lock_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_approval_required",
			entries: paneStatus.recommended_inspect_approval_required,
			shouldLog: isBoolean,
		},
		{
			prefix: "inspect_requires_code_change",
			entries: paneStatus.recommended_inspect_requires_code_change,
			shouldLog: isBoolean,
		},
		{
			prefix: "inspect_description",
			entries: paneStatus.recommended_inspect_descriptions,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_blocked_by",
			entries: paneStatus.recommended_inspect_blocked_by,
			shouldLog: hasItems,
			formatValue: joinSpace,
		},
		...
		{
			prefix: "inspect_approval_status",
			entries: paneStatus.recommended_inspect_approval_statuses,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_approval_reviewer",
			entries: paneStatus.recommended_inspect_approval_reviewers,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_approval_reason",
			entries: paneStatus.recommended_inspect_approval_reasons,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_approval_decided_at",
			entries: paneStatus.recommended_inspect_approval_decided_at,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_approval_record_present",
			entries: paneStatus.recommended_inspect_approval_record_present,
			shouldLog: isBoolean,
		},
		{ prefix: "inspect_state", entries: paneStatus.recommended_inspect_states, shouldLog: isPresentString },
		{
			prefix: "inspect_state_reason",
			entries: paneStatus.recommended_inspect_state_reasons,
			shouldLog: isPresentString,
		},
		...
		{
			prefix: "inspect_approval_path",
			entries: paneStatus.recommended_inspect_approval_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_state_dir",
			entries: paneStatus.recommended_inspect_worker_state_dirs,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_status_path",
			entries: paneStatus.recommended_inspect_worker_status_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_heartbeat_path",
			entries: paneStatus.recommended_inspect_worker_heartbeat_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_identity_path",
			entries: paneStatus.recommended_inspect_worker_identity_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_inbox_path",
			entries: paneStatus.recommended_inspect_worker_inbox_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_mailbox_path",
			entries: paneStatus.recommended_inspect_worker_mailbox_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_shutdown_request_path",
			entries: paneStatus.recommended_inspect_worker_shutdown_request_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_worker_shutdown_ack_path",
			entries: paneStatus.recommended_inspect_worker_shutdown_ack_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_dir_path",
			entries: paneStatus.recommended_inspect_team_dir_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_config_path",
			entries: paneStatus.recommended_inspect_team_config_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_manifest_path",
			entries: paneStatus.recommended_inspect_team_manifest_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_events_path",
			entries: paneStatus.recommended_inspect_team_events_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_dispatch_path",
			entries: paneStatus.recommended_inspect_team_dispatch_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_phase_path",
			entries: paneStatus.recommended_inspect_team_phase_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_monitor_snapshot_path",
			entries: paneStatus.recommended_inspect_team_monitor_snapshot_paths,
			shouldLog: isPresentString,
		},
		{
			prefix: "inspect_team_summary_snapshot_path",
			entries: paneStatus.recommended_inspect_team_summary_snapshot_paths,
			shouldLog: isPresentString,
		},
...
function formatInspectItemLine(
	index: number,
	item: TeamInspectItem,
	modelInspect = false,
...
function renderTeamPaneStatus(
	...
	if (paneStatus.sparkshell_hint) {
		console.log(
			"inspect_hint: raw tmux capture commands are quota-free; rerun status with --model-inspect for gjc sparkshell summaries",
		);
	}
...
export function parseTeamArgs(args: string[], cwd: string = process.cwd()): ParsedTeamArgs {
	...
	if (tokens[0]?.toLowerCase() === "ralph") {
		throw new Error(
			"Deprecated usage: `gjc team ralph ...` has been removed. Use `gjc team ...` or run `gjc ralph ...` separately.",
		);
	}
	...
	if (!task) {
		throw new Error('Usage: gjc team [N:agent-type] "<task description>"');
	}
	...
	const explicitApprovedCommand =
		followupContext == null
			? buildExplicitGjcTeamLaunchCommand(
					effectiveTask,
					workerCount,
					explicitWorkerCount,
					agentType,
	...
	const approvedHint =
		followupContext?.approvedHint ?? (approvedHintOutcome?.status === "resolved" ? approvedHintOutcome.hint : null);
	const matchesApprovedLaunchHint =
		followupContext == null &&
...
function createAspectSubtasks(task: string, workerCount: number): Array<{ subject: string; description: string }> {
	const aspects = [
...
async function ensureTeamModeState(parsed: ParsedTeamArgs, tasks?: Array<{ role?: string }>): Promise<void> {
	const fallbackRole = resolveImplicitTeamFallbackRole(parsed.agentType, parsed.explicitAgentType);
	const roleDistribution =
		tasks && tasks.length > 0 ? [...new Set(tasks.map(t => t.role ?? parsed.agentType))].join(",") : parsed.agentType;
...
async function persistTeamShutdownModeState(
	...
	const sessionStatePath = join(cwd, ".gjc", "state", "session.json");
	let scopedSessionId: string | undefined;
	if (existsSync(sessionStatePath)) {
		try {
			const parsed = JSON.parse(readFileSync(sessionStatePath, "utf-8")) as { session_id?: unknown };
	...
	const hasScopedState = scopedSessionId ? existsSync(getStatePath("team", cwd, scopedSessionId)) : false;
...
export function buildLeaderMonitoringHints(teamName: string): string[] {
	...
	return [
		`leader_check: gjc team status ${sanitized}`,
		`leader_loop_hint: while ON, keep checking state (example: sleep 30 && gjc team status ${sanitized})`,
	];
}
...
export async function teamCommand(args: string[], _options: TeamCliOptions = {}): Promise<void> {
	...
	if (subcommand === "api") {
		...
		if (parsedApi.json) {
			console.log(
				JSON.stringify({
					...jsonBase,
					command: `gjc team api ${parsedApi.operation}`,
					...envelope,
				}),
			);
	...
	if (subcommand === "status") {
		...
		const wantsJson = teamArgs.includes("--json");
		if (!name) throw new Error("Usage: gjc team status <team-name> [--json]");
		...
		const ultragoalCheckpointGuidance = ultragoalContext ? buildUltragoalCheckpointGuidance(ultragoalContext) : null;
		if (wantsJson) {
			console.log(
				JSON.stringify({
					...buildJsonBase(),
					...
					panes: paneStatus,
					...(ultragoalCheckpointGuidance ? { ultragoal_checkpoint_guidance: ultragoalCheckpointGuidance } : {}),
				}),
			);
			return;
		}
	...
	if (subcommand === "await") {
		const name = teamArgs[1];
		if (!name)
			throw new Error("Usage: gjc team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]");
		const wantsJson = teamArgs.includes("--json");
		...
		if (!config) {
			if (wantsJson) {
				console.log(
					JSON.stringify({ team_name: name, status: "missing", cursor: afterEventId || "", event: null }),
				);
			} else {
				console.log(`No team state found for ${name}`);
			}
		...
		const baselineCursor =
			afterEventId ||
			(await readTeamEvents(resolvedName, cwd, { wakeableOnly: true }).then(
	...
	if (subcommand === "resume") {
		const name = teamArgs[1];
		if (!name) throw new Error("Usage: gjc team resume <team-name>");
	...
	if (subcommand === "shutdown") {
		const name = teamArgs[1];
		if (!name) throw new Error("Usage: gjc team shutdown <team-name> [--force] [--confirm-issues]");
		...
		await persistTeamShutdownModeState(
			...
			console.warn("[gjc] warning: failed to persist team mode shutdown state", {
				team: name,
	...
	const effectiveParsed =
		executionPlan.workerCount === parsed.workerCount ? parsed : { ...parsed, workerCount: executionPlan.workerCount };
	const availableAgentTypes = await resolveAvailableAgentTypes(cwd);
	const staffingPlan = buildFollowupStaffingPlan("team", parsed.task, availableAgentTypes, {
	...
	await ensureTeamModeState(
		{
			...effectiveParsed,
			teamName: runtime.teamName,
			displayName: runtime.config.display_name ?? effectiveParsed.displayName,
		},
		tasks,
	);
	await renderStartSummary(runtime, staffingPlan);
