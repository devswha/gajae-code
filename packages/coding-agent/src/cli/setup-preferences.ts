...
export function getSetupScopeFilePath(projectRoot: string): string {
	return join(projectRoot, ".gjc", "setup-scope.json");
}
...
function parsePersistedSetupPreferences(
	...
	if (typeof parsed.installMode === "string" && isSetupInstallMode(parsed.installMode)) {
	...
	try {
		...
		return parsePersistedSetupPreferences(
			...
			options.warnOnLegacyScope
				? (from, to) => {
						console.warn(
							`[gjc] Migrating persisted setup scope "${from}" → "${to}" ` +
...
export function readPersistedSetupScopeSync(projectRoot: string): SetupScope | undefined {
