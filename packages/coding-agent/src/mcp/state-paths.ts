...
const WORKDIR_ALLOWLIST_ENV = "GJC_MCP_WORKDIR_ROOTS";
const GJC_ROOT_ENV = "GJC_ROOT";
const GJC_STATE_ROOT_ENV = "GJC_STATE_ROOT";
	...
	const roots = raw
		...
		.map(part => {
			...
			if (realRoot !== resolvedRoot) {
				throw new Error(
					`${WORKDIR_ALLOWLIST_ENV} root "${resolvedRoot}" resolves through a symlink to "${realRoot}"`,
				);
			}
			return realRoot;
		});
...
export function getBaseStateDir(workingDirectory?: string): string {
	const teamStateRootOverride = process.env.GJC_TEAM_STATE_ROOT?.trim();
	...
	const gjcRootOverride = process.env[GJC_ROOT_ENV]?.trim() || process.env[GJC_STATE_ROOT_ENV]?.trim();
	if (typeof gjcRootOverride === "string" && gjcRootOverride !== "") {
		try {
			return join(resolveWorkingDirectoryForState(gjcRootOverride), ".gjc", "state");
		} catch {}
	}

	return join(resolveWorkingDirectoryForState(workingDirectory), ".gjc", "state");
}
...
function readSessionIdFromEnvironment(env: NodeJS.ProcessEnv = process.env): string | undefined {
	const candidates = [env.GJC_SESSION_ID, env.CODEX_SESSION_ID, env.SESSION_ID];
...
export async function readCurrentSessionId(workingDirectory?: string): Promise<string | undefined> {
	...
	const localStateDir = join(cwd, ".gjc", "state");
 ...
 */
export async function getReadScopedStateDirs(workingDirectory?: string, explicitSessionId?: string): Promise<string[]> {
...
export async function getReadScopedStateFilePaths(
	...
	return [join(scope.stateDir, normalizedFileName), join(getBaseStateDir(workingDirectory), normalizedFileName)];
}

export async function getAllSessionScopedStatePaths(mode: string, workingDirectory?: string): Promise<string[]> {
	const sessionDirs = await getAllSessionScopedStateDirs(workingDirectory);
	const fileName = getStateFilename(mode);
	return sessionDirs.map(dir => join(dir, fileName));
}

export async function getAllScopedStatePaths(mode: string, workingDirectory?: string): Promise<string[]> {
	return [getStatePath(mode, workingDirectory), ...(await getAllSessionScopedStatePaths(mode, workingDirectory))];
}
