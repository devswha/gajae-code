/**
 * Verification Protocol for gajae-code
 *
...
export function hasStructuredVerificationEvidence(summary: string | null | undefined): boolean {
	...
	const hasVerificationSection = /verification(?:\s+evidence)?\s*:/i.test(text) || /##\s*verification/i.test(text);
 ...
 */
export function getVerificationInstructions(taskSize: "small" | "standard" | "large", taskDescription: string): string {
 ...
 */
export function determineTaskSize(fileCount: number, lineChanges: number): "small" | "standard" | "large" {
