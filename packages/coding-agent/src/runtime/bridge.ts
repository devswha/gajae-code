/**
 * TS Runtime Bridge — thin wrapper over gjc-runtime binary.
 *
 * All semantic state mutations route through `execCommand()`.
 * All state queries read Rust-authored compatibility JSON files.
 * Set GJC_RUNTIME_BRIDGE=0 to disable bridge (fallback to TS-direct).
 */
...
/**
 * Raised when the gjc-runtime binary returns output that fails JSON decoding.
 *
...
export class RuntimeBridgeError extends Error {
	...
	constructor(message: string, context: { command?: string; stdoutPreview?: string; cause?: unknown } = {}) {
		super(message);
...
export function resolveRuntimeBinaryPath(options: RuntimeBinaryDiscoveryOptions = {}): string {
	...
	const envOverride = process.env.GJC_RUNTIME_BINARY?.trim();
	if (envOverride) return envOverride;

	const workspaceDebug = options.debugPath ?? resolve(__bridge_dirname, "../../target/debug/gjc-runtime");
	if (exists(workspaceDebug)) return workspaceDebug;

	const workspaceRelease = options.releasePath ?? resolve(__bridge_dirname, "../../target/release/gjc-runtime");
	if (exists(workspaceRelease)) return workspaceRelease;

	return options.fallbackBinary ?? "gjc-runtime";
}
...
export class RuntimeBridge {
	...
	constructor(options: { stateDir?: string; binaryPath?: string } = {}) {
		this.enabled = process.env.GJC_RUNTIME_BRIDGE !== "0";
		this.stateDir = options.stateDir;
		this.binaryPath = options.binaryPath ?? resolveRuntimeBinaryPath();
	}

	/** Whether the bridge is enabled (GJC_RUNTIME_BRIDGE != '0'). */
	isEnabled(): boolean {
		return this.enabled;
	}
	...
	execCommand(cmd: RuntimeCommand, options?: { compact?: boolean }): RuntimeEvent {
		...
		try {
			...
			throw new RuntimeBridgeError(`gjc-runtime exec returned non-JSON output for ${cmd.command}`, {
				command: cmd.command,
	...
	readSnapshot(): RuntimeSnapshot {
		...
		try {
			...
			throw new RuntimeBridgeError("gjc-runtime snapshot returned non-JSON output", {
				command: "snapshot",
				stdoutPreview: stdout.slice(0, 200),
				cause,
			});
	...
	private validateSchemaOnce(): void {
		...
		try {
			...
			if (missing.length > 0) {
				throw new Error(
					`gjc-runtime schema missing commands: ${missing.join(", ")}. ` +
	...
	private run(args: string[]): string {
		try {
			...
			throw new Error(`gjc-runtime ${args[0]} failed: ${stderr}`);
		}
	}
}
...
export function isBridgeEnabled(): boolean {
	return process.env.GJC_RUNTIME_BRIDGE !== "0";
}
