...
export const GOAL_WORKFLOWS_DIR = ".gjc/goals";
...
export function goalWorkflowDir(cwd: string, workflow: string, slug: string): string {
	return join(
		cwd,
		GOAL_WORKFLOWS_DIR,
		normalizeGoalWorkflowSegment(workflow),
		normalizeGoalWorkflowSegment(slug, "goal-workflow"),
	);
}
...
export async function appendGoalWorkflowLedger(
	cwd: string,
	run: GoalWorkflowRun,
	entry: GoalWorkflowLedgerEntry,
): Promise<void> {
...
export async function createGoalWorkflowRun(
	cwd: string,
	options: CreateGoalWorkflowRunOptions,
): Promise<GoalWorkflowRun> {
	...
	if (!options.force && existsSync(statusPath)) {
		throw new GoalWorkflowError(
			`Refusing to overwrite existing ${repoRelative(cwd, statusPath)}; pass force to recreate it.`,
		);
	}
...
export async function transitionGoalWorkflowRun(
	cwd: string,
	workflow: string,
	slug: string,
	options: TransitionGoalWorkflowOptions,
): Promise<GoalWorkflowRun> {
