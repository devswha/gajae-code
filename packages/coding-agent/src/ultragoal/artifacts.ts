...
import {
	formatGJCGoalReconciliation,
	parseGJCGoalSnapshot,
	reconcileGJCGoalSnapshot,
} from "../goal-workflows/gjc-goal-snapshot.js";

export const ULTRAGOAL_DIR = ".gjc/ultragoal";
...
export type UltragoalStatus =
	...
	| "review_blocked"
	| "needs_user_decision";
export type UltragoalGJCGoalMode = "aggregate" | "per_story";
...
export const ULTRAGOAL_STEERING_SOURCES: readonly UltragoalSteeringSource[] = ["user_prompt_submit", "finding", "cli"];
...
export interface CheckpointOptions {
	...
	gjcGoal?: unknown;
	qualityGate?: unknown;
	allowActiveFinalGJCGoal?: boolean;
	now?: Date;
}
...
function parseMarkdownListItems(lines: readonly string[]): MarkdownListItem[] {
	...
	for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
		...
		const nextSection =
			normalizeSectionLabel(line) ?? (afterBlank ? normalizeIndentedAtxStorySectionLabel(line) : undefined);
		if (nextSection) {
			section = nextSection;
...
function selectedItemObjective(
	parent: MarkdownListItem,
	lines: readonly string[],
	nextParentLineIndex?: number,
): string {
...
function classifyExternalAuthorizationBlocker(evidence: string | undefined): ExternalAuthorizationBlocker | null {
	...
	const mentionsAuthorization =
		/\b(auth|authorization|credential|credentials|token|permission|permissions|scope|scopes|access|unauthorized|forbidden|401|403)\b/.test(
			normalized,
		);
	const mentionsMissingAuthority =
		/\b(unset|missing|required|requires|without|omit|omits|not set|not available|no read packages|read packages)\b/.test(
			normalized,
		);
	if (!mentionsAuthorization || !mentionsMissingAuthority) return null;

	const mentionsGhcr =
		/\b(ghcr|github container registry|read packages|imagepullsecret|package api|anonymous image|container image)\b/.test(
			normalized,
		);
	if (mentionsGhcr) {
		...
		const status =
			[has401 ? "HTTP_401_ANONYMOUS" : null, has403 ? "HTTP_403_NO_READ_PACKAGES" : null]
				.filter((part): part is string => Boolean(part))
				.join("+") || "AUTHORIZATION_REQUIRED";
		return {
			signature: `GHCR_PULL_ACCESS:${status}:GHCR_VISIBILITY_OR_CREDENTIAL_REQUIRED`,
			requiredDecision:
				"make the GHCR package public, or provide/authorize a least-privilege read:packages credential and imagePullSecret/SOPS path",
		};
	}
	...
	return {
		...
		requiredDecision:
			"provide the missing external authorization/credential, or explicitly choose a different unblock path",
	};
}
...
async function canReconcileCompletedTaskScopedAggregateSnapshot(
	...
	if (gjcGoalMode(plan) !== "aggregate") return false;
...
function buildCompletedLegacyGoalRemediation(goal: UltragoalItem): string {
	return [
		...
		`Record a non-terminal blocker with: gjc ultragoal checkpoint --goal-id ${goal.id} --status blocked --evidence "<completed legacy GJC goal blocks create_goal in this thread>" --gjc-goal-json "<different completed get_goal JSON or path>".`,
		"Then continue only from a GJC goal context with no active/completed conflicting goal, in the same repo/worktree, and create the intended goal there.",
	].join(" ");
}

function buildUnavailableGJCGoalRemediation(goal: UltragoalItem): string {
	return [
		'If get_goal itself is unavailable due to a GJC DB/schema/context error, such as "no such table: thread_goals", do not repeat --status complete or mark the GJC goal complete from shell state.',
		`Record an auditable non-terminal blocker with: gjc ultragoal checkpoint --goal-id ${goal.id} --status blocked --evidence "<get_goal unavailable due to GJC DB/schema/context error; safe recovery requires a working GJC goal context>" --gjc-goal-json "<unavailable get_goal error JSON or path>".`,
		"Then continue from a GJC goal context where get_goal works and strict completion reconciliation can be proven.",
	].join(" ");
}
...
function evidenceDescribesCompletedAggregateMicrogoalLoop(evidence: string | undefined): boolean {
	...
	return (
		normalized.includes("aggregate gjc goal") &&
...
function isSafeCompletedAggregateBlockerSnapshot(
	plan: UltragoalPlan,
	goal: UltragoalItem,
	snapshot: ReturnType<typeof parseGJCGoalSnapshot>,
	evidence: string | undefined,
): boolean {
	if (gjcGoalMode(plan) !== "aggregate") return false;
	if (goal.status !== "in_progress" || plan.activeGoalId !== goal.id) return false;
	...
	return [expectedGJCObjective(plan, goal), ...compatibleGJCObjectives(plan)].some(
		objective => normalizeObjective(objective) === actual,
	);
}

function gjcGoalMode(plan: UltragoalPlan): UltragoalGJCGoalMode {
	return plan.gjcGoalMode ?? "per_story";
}
...
export const ULTRAGOAL_AGGREGATE_GJC_OBJECTIVE = `Complete the durable ultragoal plan in ${ULTRAGOAL_DIR}/${ULTRAGOAL_GOALS}, including later accepted/appended stories, under the original brief constraints; use ${ULTRAGOAL_DIR}/${ULTRAGOAL_LEDGER} as the audit trail.`;

function aggregateGJCObjective(_goals: readonly UltragoalItem[]): string {
	if (ULTRAGOAL_AGGREGATE_GJC_OBJECTIVE.length <= 4000) return ULTRAGOAL_AGGREGATE_GJC_OBJECTIVE;
	throw new UltragoalError("Generated aggregate GJC objective exceeds the 4,000 character goal limit.");
}
...
function isLegacyEnumeratedAggregateObjective(objective: string | undefined): boolean {
	...
	return (
		...
		objective ===
			`Complete all ultragoal stories listed in ${ULTRAGOAL_DIR}/${ULTRAGOAL_GOALS}. Use ${ULTRAGOAL_DIR}/${ULTRAGOAL_LEDGER} as the durable audit trail.`
	);
}

function compatibleGJCObjectives(plan: UltragoalPlan): string[] {
	return (plan.gjcObjectiveAliases ?? []).filter(objective => isLegacyEnumeratedAggregateObjective(objective));
}

function expectedGJCObjective(plan: UltragoalPlan, goal: UltragoalItem): string {
	return gjcGoalMode(plan) === "aggregate" ? (plan.gjcObjective ?? aggregateGJCObjective(plan.goals)) : goal.objective;
}
...
export function isUltragoalDone(plan: UltragoalPlan): boolean {
	...
	const latestNonReviewBlocked = [...plan.goals]
		.reverse()
		.find(goal => goal.status !== "review_blocked" && goal.steeringStatus !== "superseded");
...
export function deriveGoalCandidates(brief: string): Array<{ title: string; objective: string }> {
	...
	const bulletGoals =
		listGoals.length > 0 || parsedItems.length > 0
			? listGoals
			: lines
...
export async function readUltragoalPlan(cwd: string): Promise<UltragoalPlan> {
	...
	try {
		...
		throw new UltragoalError(
			`No ultragoal plan found at ${repoRelative(cwd, path)}. Run \`gjc ultragoal create-goals ...\` first.`,
		);
	}
	...
	if (gjcGoalMode(parsed) === "aggregate" && isLegacyEnumeratedAggregateObjective(parsed.gjcObjective)) {
		const previousObjective = parsed.gjcObjective;
		const now = iso();
		parsed.gjcObjective = aggregateGJCObjective(parsed.goals);
		parsed.gjcObjectiveAliases = Array.from(
			new Set(
				[...(parsed.gjcObjectiveAliases ?? []), previousObjective].filter(
		...
		await appendLedger(cwd, {
			...
			message: "Migrated legacy enumerated aggregate GJC objective to the stable pointer objective.",
			before: { gjcObjective: previousObjective },
			after: { gjcObjective: parsed.gjcObjective },
		});
	}
	return parsed;
}
...
export async function createUltragoalPlan(cwd: string, options: CreateUltragoalOptions): Promise<UltragoalPlan> {
	return withUltragoalMutationLock(cwd, async () => {
		...
		const plan: UltragoalPlan = {
			...
			gjcGoalMode: options.gjcGoalMode ?? "aggregate",
			goals: candidates,
		};
		if (plan.gjcGoalMode === "aggregate") plan.gjcObjective = aggregateGJCObjective(candidates);
...
function proposalTargetIds(proposal: UltragoalSteeringProposal): string[] {
	return proposal.targetGoalIds?.length
...
function mentionsWeakenedCompletion(...values: Array<string | undefined>): boolean {
	...
	return (
		/\b(skip|bypass|weaken|remove|omit|auto[-\s]?complete|mark complete|complete faster)\b/.test(normalized) &&
...
export function validateUltragoalSteeringProposal(
	plan: UltragoalPlan,
	proposal: UltragoalSteeringProposal,
): UltragoalSteeringInvariantResult {
	...
	if (hasProtectedSteeringPayload(proposal.after))
		rejectedReasons.push(
			"Steering payload must not edit protected objective, constraint, quality gate, or completion fields.",
		);
	if (
		/\b(?:skip|bypass|weaken|remove)\b.*\b(?:test|tests|review|verification|quality gate|complete|completion)\b|\bauto[- ]?complete\b/.test(
			protectedIntentText(proposal),
		)
	) {
		rejectedReasons.push(
			"Steering must not weaken completion, quality gates, tests, reviews, or auto-complete work.",
		);
	}
	...
	if (
		(proposal.kind === "split_subgoal" ||
			proposal.kind === "revise_pending_wording" ||
	...
	if (
		(proposal.kind === "split_subgoal" || proposal.kind === "revise_pending_wording") &&
	...
	if (proposal.kind === "reorder_pending") {
		...
		const pending = plan.goals
			.filter(goal => goal.status === "pending" && isScheduleEligible(goal))
			.map(goal => goal.id);
...
function moveGoalsAfterTarget(plan: UltragoalPlan, targetId: string, movedIds: string[]): void {
	const moved = movedIds
		.map(id => plan.goals.find(goal => goal.id === id))
		.filter((goal): goal is UltragoalItem => Boolean(goal));
...
function applySteeringMutation(
	plan: UltragoalPlan,
	proposal: UltragoalSteeringProposal,
	now: string,
	...
	const target = targets[0];
	if (proposal.kind === "add_subgoal") {
		const goal = appendGoalToPlan(plan, {
	...
	if (proposal.kind === "split_subgoal") {
		...
		const children = childGoalsFromProposal(proposal).map(child =>
			appendGoalToPlan(plan, { ...child, evidence: proposal.evidence, now: new Date(now) }),
		);
		target.steeringStatus = "superseded";
		target.supersededBy = children.map(child => child.id);
		moveGoalsAfterTarget(
			plan,
			target.id,
	...
	if (proposal.kind === "reorder_pending") {
		...
		const requestedGoals = requested
			.map(id => plan.goals.find(goal => goal.id === id))
			.filter((goal): goal is UltragoalItem => Boolean(goal));
	...
	if (proposal.kind === "mark_blocked_superseded") {
		...
		if (children.length > 0) {
			const replacements = children.map(child =>
				appendGoalToPlan(plan, { ...child, evidence: proposal.evidence, now: new Date(now) }),
			);
			target.steeringStatus = "superseded";
			...
			moveGoalsAfterTarget(
				plan,
				target.id,
				replacements.map(child => child.id),
			);
			target.steeringEvidence = proposal.evidence;
			target.steeringRationale = proposal.rationale;
			target.updatedAt = now;
...
export async function steerUltragoal(
	cwd: string,
	proposal: UltragoalSteeringProposal,
	options: { now?: Date; directiveText?: string } = {},
): Promise<SteerUltragoalResult> {
	return withUltragoalMutationLock(cwd, async () => {
		const plan = await readUltragoalPlan(cwd);
		const existing = proposal.idempotencyKey
			? (await readSteeringLedgerEntries(cwd)).find(
					entry =>
						entry.event === "steering_accepted" &&
						(entry.idempotencyKey === proposal.idempotencyKey ||
							entry.steering?.idempotencyKey === proposal.idempotencyKey) &&
						entry.steering,
				)
			: undefined;
		if (existing?.steering) {
			return {
				plan,
				accepted: true,
				audit: { ...existing.steering, deduped: true },
				rejectedReasons: [],
				deduped: true,
			};
		}
...
function validateQualityGate(value: unknown): UltragoalQualityGate {
	...
	if (!independentReview || typeof independentReview !== "object") {
		throw new UltragoalError(
			"Final code-review independent review unavailable: codeReview.independentReview must include completed code-reviewer and architect subagent evidence; use record-review-blockers instead of self-approving.",
		);
	}
	const codeReviewer = independentReview.codeReviewer;
	if (!codeReviewer || typeof codeReviewer !== "object") {
		throw new UltragoalError(
			"Final code-review independent review unavailable: missing codeReview.independentReview.codeReviewer evidence from the code-reviewer subagent.",
		);
	}
	if (codeReviewer.agentRole !== "code-reviewer") {
		throw new UltragoalError(
			"Final code-review must use an independent code-reviewer subagent; self-review or default/authoring-lane review cannot approve the ultragoal gate.",
		);
	}
	...
	if (!architect || typeof architect !== "object") {
		throw new UltragoalError(
			"Final code-review independent review unavailable: missing codeReview.independentReview.architect evidence from the architect subagent.",
		);
	}
...
): Promise<{ plan: UltragoalPlan; goal: UltragoalItem | null; resumed: boolean; done: boolean }> {
	return withUltragoalMutationLock(cwd, async () => {
		const plan = await readUltragoalPlan(cwd);
		const now = iso(options.now);
...
export async function checkpointUltragoal(cwd: string, options: CheckpointOptions): Promise<UltragoalPlan> {
	return withUltragoalMutationLock(cwd, async () => {
		...
		if (options.status === "blocked") {
			...
			const snapshot = options.gjcGoal === undefined ? null : parseGJCGoalSnapshot(options.gjcGoal);
			if (snapshot?.unavailableReason === "db_schema_context_error") {
				...
				await appendLedger(cwd, {
					...
					status: goal.status,
					evidence: options.evidence,
					gjcGoal: options.gjcGoal,
					message:
						"GJC get_goal was unavailable due to a DB/schema/context error; strict completion reconciliation is deferred until get_goal works.",
				});
				return plan;
			}
			if (!snapshot?.available) {
				throw new UltragoalError(
					"Blocked ultragoal checkpoints require either a get_goal snapshot for the completed legacy GJC goal that blocked create_goal, or an unavailable get_goal error JSON for a GJC DB/schema/context failure; pass --gjc-goal-json.",
				);
			}
			if (snapshot.status !== "complete") {
				throw new UltragoalError(
					`Cannot record a blocked ultragoal checkpoint while the existing GJC goal is ${snapshot.status ?? "unknown"}; strict objective mismatch protection remains required for active or incomplete goals.`,
				);
			}
			if (!snapshot.objective) {
				throw new UltragoalError("Blocked ultragoal checkpoint GJC snapshot is missing objective text.");
			}
			const safeCompletedAggregateBlocker = isSafeCompletedAggregateBlockerSnapshot(
				plan,
				goal,
				snapshot,
				options.evidence,
			);
			const blockedSnapshotMatchesExpected = [
				expectedGJCObjective(plan, goal),
				...compatibleGJCObjectives(plan),
			...
			if (!safeCompletedAggregateBlocker && blockedSnapshotMatchesExpected) {
				throw new UltragoalError(
					"Blocked ultragoal checkpoint is only for a different completed legacy GJC goal unless an aggregate GJC goal is already complete and unreconcilable while the active repo-native microgoal remains in progress.",
				);
			}
			goal.updatedAt = now;
			...
			await appendLedger(cwd, {
				...
				message: safeCompletedAggregateBlocker
					? "Completed aggregate GJC goal is already terminal while the repo-native microgoal remains in progress; recorded a non-terminal safe-recovery blocker to avoid repeating an impossible checkpoint loop."
					: undefined,
			});
			return plan;
		}
		...
		if (options.status === "complete") {
			const expectedObjective = expectedGJCObjective(plan, goal);
			const aggregateMode = gjcGoalMode(plan) === "aggregate";
			const finalRunCheckpoint = isFinalRunCompletionCandidate(plan, goal);
			const snapshot = options.gjcGoal === undefined ? null : parseGJCGoalSnapshot(options.gjcGoal);
			const reconciliation = reconcileGJCGoalSnapshot(snapshot, {
				expectedObjective,
				acceptedObjectives: aggregateMode ? compatibleGJCObjectives(plan) : undefined,
				allowedStatuses: aggregateMode
					? finalRunCheckpoint && !options.allowActiveFinalGJCGoal
						? ["complete"]
						: ["active"]
					: ["complete"],
				requireSnapshot: true,
				requireComplete: !aggregateMode || (finalRunCheckpoint && !options.allowActiveFinalGJCGoal),
			});
			if (!reconciliation.ok) {
				...
				if (completedTaskScopedAggregateSnapshot) {
					...
					const taskScopedRequirement =
						aggregateMode && snapshot?.status === "complete" && snapshot.objective
							? " Completed task-scoped aggregate reconciliation requires the checkpoint goal to be the active in-progress GJC goal, evidence that names that active GJC goal id, names .gjc/ultragoal/goals.json or ledger.jsonl, includes completed implementation plus validation/review evidence, and a get_goal objective that maps to the ultragoal brief/artifact."
							: "";
					const remediation =
						reconciliation.snapshot.available &&
						reconciliation.snapshot.status === "complete" &&
						reconciliation.snapshot.objective &&
						normalizeObjective(reconciliation.snapshot.objective ?? "") !== normalizeObjective(expectedObjective)
							...
							: reconciliation.snapshot.unavailableReason === "db_schema_context_error"
								? ` ${buildUnavailableGJCGoalRemediation(goal)}`
								: "";
					throw new UltragoalError(
						`${formatGJCGoalReconciliation(reconciliation)}${taskScopedRequirement}${remediation}`,
					);
				}
			}
			if (finalRunCheckpoint && !options.allowActiveFinalGJCGoal) goal.evidence = options.evidence;
		}
		const qualityGate =
			options.status === "complete" &&
			(aggregateCompletion !== undefined ||
				(isFinalRunCompletionCandidate(plan, goal) && !options.allowActiveFinalGJCGoal))
		...
		if (aggregateCompletion) {
			...
			await appendLedger(cwd, {
				...
				qualityGate,
				message:
					"Aggregate ultragoal plan completed via task-scoped GJC goal snapshot; microgoal ledger progress remains independent.",
			});
			return plan;
		}
...
): Promise<{ plan: UltragoalPlan; blockedGoal: UltragoalItem; addedGoal: UltragoalItem }> {
	return withUltragoalMutationLock(cwd, async () => {
		const plan = await readUltragoalPlan(cwd);
		const goal = plan.goals.find(candidate => candidate.id === options.goalId);
		if (!goal) throw new UltragoalError(`Unknown ultragoal id: ${options.goalId}`);
		...
		const expectedObjective = expectedGJCObjective(plan, goal);
		const aggregateMode = gjcGoalMode(plan) === "aggregate";
		const reconciliation = reconcileGJCGoalSnapshot(
			options.gjcGoal === undefined ? null : parseGJCGoalSnapshot(options.gjcGoal),
			{
				expectedObjective,
				acceptedObjectives: aggregateMode ? compatibleGJCObjectives(plan) : undefined,
		...
		if (!reconciliation.ok) {
			throw new UltragoalError(formatGJCGoalReconciliation(reconciliation));
		}
		...
		await appendLedger(cwd, {
			...
			message: aggregateMode
				? "Final aggregate code-review was not clean; blocker story was appended while GJC goal remains active."
				: "Final per-story code-review was not clean; blocker story was appended and may require an available GJC goal context.",
		});
		await appendLedger(cwd, {
			ts: now,
			event: "goal_added",
...
export function buildGJCGoalInstruction(goal: UltragoalItem, plan: UltragoalPlan): string {
	if (gjcGoalMode(plan) === "aggregate") return buildAggregateGJCGoalInstruction(goal, plan);
	return buildPerStoryGJCGoalInstruction(goal, plan);
}

function buildPerStoryGJCGoalInstruction(goal: UltragoalItem, plan: UltragoalPlan): string {
	const createPayload = {
	...
	return [
		...
		"- If a different active GJC goal exists, finish/checkpoint that goal before starting this ultragoal.",
		"- Ultragoal cannot call /goal clear from the model/shell tool surface. For another per-story goal in the same session/thread after a completed GJC goal, manually run /goal clear in the GJC UI before creating the next goal.",
		"- If get_goal returns a different completed legacy/thread goal and create_goal rejects because this thread already has a completed goal, continue only from a GJC goal context with no active/completed conflicting goal in the same repo/worktree and create the payload there.",
		`- To preserve the durable ledger before switching threads, record the non-terminal blocker without failing this goal: gjc ultragoal checkpoint --goal-id ${goal.id} --status blocked --evidence "<completed legacy GJC goal blocks create_goal in this thread>" --gjc-goal-json "<get_goal JSON or path>"`,
		...
		finalStory
			? `  gjc ultragoal record-review-blockers --goal-id ${goal.id} --title "Resolve final code-review blockers" --objective "<blocker-resolution objective>" --evidence "<review findings>" --gjc-goal-json "<active get_goal JSON or path>"`
			: `  gjc ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --gjc-goal-json "<fresh get_goal JSON or path>"`,
		finalStory
			? "- In legacy per-story mode, the blocker story may require an available GJC goal context because this story remains an active incomplete GJC goal; do not claim it is complete."
			: null,
		finalStory
			...
			: null,
		finalStory
			? `  gjc ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --gjc-goal-json "<fresh complete get_goal JSON or path>" --quality-gate-json "<quality gate JSON or path>"`
			: null,
...
function buildAggregateGJCGoalInstruction(goal: UltragoalItem, plan: UltragoalPlan): string {
	const objective = plan.gjcObjective ?? aggregateGJCObjective(plan.goals);
	const finalStory = isFinalRunCompletionCandidate(plan, goal);
	...
	return [
		...
		"",
		"GJC goal integration constraints:",
		"- GJC goal = the whole ultragoal run; GJC G001/G002/etc. = ledger stories.",
		"- First call get_goal. If no active goal exists, call create_goal with the aggregate payload below.",
		"- If get_goal reports the same aggregate objective as active, continue this GJC story without creating a new GJC goal.",
		"- If a different active or incomplete GJC goal exists, finish/checkpoint that goal before starting this ultragoal; do not replace hidden GJC state from the shell.",
		"- Ultragoal does not call /goal clear. After a completed aggregate run, manually run /goal clear in the GJC UI before starting another ultragoal run in the same session/thread.",
		finalStory
			...
			: "- This is not the final story: do not call update_goal yet; the aggregate GJC goal must remain active while later GJC stories remain.",
		finalStory
			? "- Final $code-review is clean only when it is APPROVE with architect status CLEAR and includes independentReview evidence from both code-reviewer and architect subagents."
			: null,
		finalStory
		...
		finalStory
			...
			: null,
		`- Checkpoint this GJC story with a fresh get_goal snapshot whose objective matches the aggregate payload and whose status is ${checkpointStatus}:`,
		finalStory
			? `  gjc ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --gjc-goal-json "<fresh complete get_goal JSON or path>" --quality-gate-json "<quality gate JSON or path>"`
			: `  gjc ultragoal checkpoint --goal-id ${goal.id} --status complete --evidence "<tests/files/PR evidence>" --gjc-goal-json "<fresh get_goal JSON or path>"`,
		"- If blocked or failed, checkpoint with --status failed and the failure evidence; rerun complete-goals --retry-failed to resume.",
